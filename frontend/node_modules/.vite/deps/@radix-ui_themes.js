...
function composeRefs(...refs) {
  return (node) => {
    ...
    if (hasCleanup) {
      return () => {
        for (let i18 = 0; i18 < cleanups.length; i18++) {
          const cleanup = cleanups[i18];
          if (typeof cleanup == "function") {
            cleanup();
          } else {
            setRef(refs[i18], null);
          }
        }
      };
    }
  ...
  function useCollection10(scope) {
    ...
    const getItems = import_react.default.useCallback(() => {
      ...
      const orderedItems = items.sort(
        (a22, b6) => orderedNodes.indexOf(a22.ref.current) - orderedNodes.indexOf(b6.ref.current)
      );
      return orderedItems;
...
var OrderedDict = class _OrderedDict extends Map {
  ...
  insert(index2, key, value) {
    ...
    for (let i18 = actualIndex; i18 < size4; i18++) {
      if (actualIndex === i18) {
        let nextKey = keys[i18];
        if (keys[i18] === key) {
          nextKey = keys[i18 + 1];
        }
        if (has) {
        ...
        if (!shouldSkip && keys[i18 - 1] === key) {
          shouldSkip = true;
        }
        const currentKey = keys[shouldSkip ? i18 : i18 - 1];
...
function shallowEqual(a22, b6) {
  if (a22 === b6) return true;
  if (typeof a22 !== "object" || typeof b6 !== "object") return false;
  if (a22 == null || b6 == null) return false;
  const keysA = Object.keys(a22);
  const keysB = Object.keys(b6);
  if (keysA.length !== keysB.length) return false;
  for (const key of keysA) {
    if (!Object.prototype.hasOwnProperty.call(b6, key)) return false;
    if (a22[key] !== b6[key]) return false;
  }
  return true;
}
function isElementPreceding(a22, b6) {
  return !!(b6.compareDocumentPosition(a22) & Node.DOCUMENT_POSITION_PRECEDING);
}
function sortByDocumentPosition(a22, b6) {
  return !a22[1].element || !b6[1].element ? 0 : isElementPreceding(a22[1].element, b6[1].element) ? -1 : 1;
}
...
var __assign = function() {
  __assign = Object.assign || function __assign2(t16) {
    for (var s20, i18 = 1, n20 = arguments.length; i18 < n20; i18++) {
      s20 = arguments[i18];
      for (var p26 in s20) if (Object.prototype.hasOwnProperty.call(s20, p26)) t16[p26] = s20[p26];
    }
    return t16;
  };
  return __assign.apply(this, arguments);
};
function __rest(s20, e28) {
  var t16 = {};
  for (var p26 in s20) if (Object.prototype.hasOwnProperty.call(s20, p26) && e28.indexOf(p26) < 0)
    t16[p26] = s20[p26];
  if (s20 != null && typeof Object.getOwnPropertySymbols === "function")
    for (var i18 = 0, p26 = Object.getOwnPropertySymbols(s20); i18 < p26.length; i18++) {
      if (e28.indexOf(p26[i18]) < 0 && Object.prototype.propertyIsEnumerable.call(s20, p26[i18]))
        t16[p26[i18]] = s20[p26[i18]];
    }
  return t16;
}
function __spreadArray(to, from, pack) {
  if (pack || arguments.length === 2) for (var i18 = 0, l10 = from.length, ar; i18 < l10; i18++) {
    if (ar || !(i18 in from)) {
...
function RemoveScrollSideCar(props) {
  ...
  var shouldPrevent = React32.useCallback(function(_event) {
    ...
    var sourceEvent = shouldPreventQueue.current.filter(function(e28) {
      return e28.name === event.type && (e28.target === event.target || event.target === e28.shadowParent) && deltaCompare(e28.delta, delta);
    })[0];
  ...
  var shouldCancel = React32.useCallback(function(name, delta, target, should) {
    ...
    setTimeout(function() {
      shouldPreventQueue.current = shouldPreventQueue.current.filter(function(e28) {
        return e28 !== event;
      });
    }, 1);
  }, []);
    ...
    Array.prototype.forEach.call(parent.children, function(node) {
      if (elementsToKeep.has(node)) {
        ...
        try {
          ...
          console.error("aria-hidden: cannot operate on ", node, e28);
        }
      }
    });
  };
  ...
  return {
    ...
    async fn(state) {
      ...
      if (!overflows.every((side2) => side2 <= 0)) {
        ...
        let resetPlacement = (_overflowsData$filter = overflowsData.filter((d10) => d10.overflows[0] <= 0).sort((a22, b6) => a22.overflows[1] - b6.overflows[1])[0]) == null ? void 0 : _overflowsData$filter.placement;
        if (!resetPlacement) {
          switch (fallbackStrategy) {
            case "bestFit": {
              ...
              const placement2 = (_overflowsData$filter2 = overflowsData.filter((d10) => {
                ...
                return true;
              }).map((d10) => [d10.placement, d10.overflows.filter((overflow2) => overflow2 > 0).reduce((acc, overflow2) => acc + overflow2, 0)]).sort((a22, b6) => a22[1] - b6[1])[0]) == null ? void 0 : _overflowsData$filter2[0];
              if (placement2) {
...
function rectsAreEqual(a22, b6) {
  return a22.x === b6.x && a22.y === b6.y && a22.width === b6.width && a22.height === b6.height;
}
...
function deepEqual(a22, b6) {
  if (a22 === b6) {
    return true;
  }
  if (typeof a22 !== typeof b6) {
    return false;
  }
  if (typeof a22 === "function" && a22.toString() === b6.toString()) {
    return true;
  }
  let length;
  let i18;
  let keys;
  if (a22 && b6 && typeof a22 === "object") {
    if (Array.isArray(a22)) {
      length = a22.length;
      if (length !== b6.length) return false;
      for (i18 = length; i18-- !== 0; ) {
        if (!deepEqual(a22[i18], b6[i18])) {
          return false;
        }
      }
      return true;
    }
    keys = Object.keys(a22);
    length = keys.length;
    if (length !== Object.keys(b6).length) {
      return false;
    }
    for (i18 = length; i18-- !== 0; ) {
      if (!{}.hasOwnProperty.call(b6, keys[i18])) {
        return false;
      }
    }
    for (i18 = length; i18-- !== 0; ) {
      const key = keys[i18];
      if (key === "_owner" && a22.$$typeof) {
        continue;
      }
      if (!deepEqual(a22[key], b6[key])) {
        return false;
      }
    }
    return true;
  }
  return a22 !== a22 && b6 !== b6;
}
...
function isPointInPolygon(point, polygon) {
  ...
  let inside = false;
  for (let i18 = 0, j = polygon.length - 1; i18 < polygon.length; j = i18++) {
    const ii = polygon[i18];
    const jj = polygon[j];
    const xi = ii.x;
    const yi = ii.y;
    const xj = jj.x;
    ...
    const dispatch = useEffectEvent((action) => {
      switch (action.type) {
        ...
        case "CLEAR_CHAR": {
          ...
          const newValue = value.filter((_, i18) => i18 !== index2);
          const currentTarget = collection.at(index2)?.element;
...
function getNextSortedValues(prevValues = [], nextValue, atIndex) {
  ...
  return nextValues.sort((a22, b6) => a22 - b6);
}
...
function getHull(points) {
  ...
  newPoints.sort((a22, b6) => {
    if (a22.x < b6.x) return -1;
    else if (a22.x > b6.x) return 1;
    else if (a22.y < b6.y) return -1;
    else if (a22.y > b6.y) return 1;
    else return 0;
  });
...
function getHullPresorted(points) {
  ...
  const upperHull = [];
  for (let i18 = 0; i18 < points.length; i18++) {
    const p26 = points[i18];
    while (upperHull.length >= 2) {
      const q = upperHull[upperHull.length - 1];
      const r24 = upperHull[upperHull.length - 2];
      if ((q.x - r24.x) * (p26.y - r24.y) >= (q.y - r24.y) * (p26.x - r24.x)) upperHull.pop();
      else break;
    }
    upperHull.push(p26);
  }
  upperHull.pop();
  const lowerHull = [];
  for (let i18 = points.length - 1; i18 >= 0; i18--) {
    const p26 = points[i18];
    while (lowerHull.length >= 2) {
      const q = lowerHull[lowerHull.length - 1];
      const r24 = lowerHull[lowerHull.length - 2];
      if ((q.x - r24.x) * (p26.y - r24.y) >= (q.y - r24.y) * (p26.x - r24.x)) lowerHull.pop();
      else break;
    }
    lowerHull.push(p26);
  }
...
function e5(n20, r24) {
  return Object.prototype.hasOwnProperty.call(n20, r24);
}

// node_modules/@radix-ui/themes/dist/esm/helpers/is-responsive-object.js
function i5(e28) {
  return typeof e28 == "object" && Object.keys(e28).some((s20) => e2.includes(s20));
}

// node_modules/@radix-ui/themes/dist/esm/helpers/get-responsive-styles.js
function R({ className: r24, customProperties: n20, ...t16 }) {
  const p26 = g({ allowArbitraryValues: true, className: r24, ...t16 }), e28 = m3({ customProperties: n20, ...t16 });
  return [p26, e28];
}
function g({ allowArbitraryValues: r24, value: n20, className: t16, propValues: p26, parseValue: e28 = (s20) => s20 }) {
  const s20 = [];
  if (n20) {
    if (typeof n20 == "string" && p26.includes(n20)) return l(t16, n20, e28);
    if (i5(n20)) {
      const i18 = n20;
      for (const o42 in i18) {
        if (!e5(i18, o42) || !e2.includes(o42)) continue;
        const u9 = i18[o42];
        if (u9 !== void 0) {
          if (p26.includes(u9)) {
            const f18 = l(t16, u9, e28), v11 = o42 === "initial" ? f18 : `${o42}:${f18}`;
            s20.push(v11);
          } else if (r24) {
            const f18 = o42 === "initial" ? t16 : `${o42}:${t16}`;
            s20.push(f18);
          }
        }
      }
...
function l(r24, n20, t16) {
  const p26 = r24 ? "-" : "", e28 = t16(n20), s20 = e28?.startsWith("-"), i18 = s20 ? "-" : "", o42 = s20 ? e28?.substring(1) : e28;
  return `${i18}${r24}${p26}${o42}`;
}
function m3({ customProperties: r24, value: n20, propValues: t16, parseValue: p26 = (e28) => e28 }) {
  let e28 = {};
  if (!(!n20 || typeof n20 == "string" && t16.includes(n20))) {
    if (typeof n20 == "string" && (e28 = Object.fromEntries(r24.map((s20) => [s20, n20]))), i5(n20)) {
      const s20 = n20;
      for (const i18 in s20) {
        if (!e5(s20, i18) || !e2.includes(i18)) continue;
        const o42 = s20[i18];
        if (!t16.includes(o42)) for (const u9 of r24) e28 = { [i18 === "initial" ? u9 : `${u9}-${i18}`]: o42, ...e28 };
      }
    }
    for (const s20 in e28) {
      const i18 = e28[s20];
      i18 !== void 0 && (e28[s20] = p26(i18));
    }
    return e28;
  }
}
...
function l2(...t16) {
  let e28 = {};
  for (const n20 of t16) n20 && (e28 = { ...e28, ...n20 });
  return Object.keys(e28).length ? e28 : void 0;
}

// node_modules/@radix-ui/themes/dist/esm/helpers/extract-props.js
function N(...r24) {
  return Object.assign({}, ...r24);
}
function v(r24, ...m13) {
  let t16, l10;
  const a22 = { ...r24 }, f18 = N(...m13);
  for (const n20 in f18) {
    let s20 = a22[n20];
    const e28 = f18[n20];
    if (e28.default !== void 0 && s20 === void 0 && (s20 = e28.default), e28.type === "enum" && ![e28.default, ...e28.values].includes(s20) && !i5(s20) && (s20 = e28.default), a22[n20] = s20, "className" in e28 && e28.className) {
      delete a22[n20];
      const u9 = "responsive" in e28;
      if (!s20 || i5(s20) && !u9) continue;
      if (i5(s20) && (e28.default !== void 0 && s20.initial === void 0 && (s20.initial = e28.default), e28.type === "enum" && ([e28.default, ...e28.values].includes(s20.initial) || (s20.initial = e28.default))), e28.type === "enum") {
        const i18 = g({ allowArbitraryValues: false, value: s20, className: e28.className, propValues: e28.values, parseValue: e28.parseValue });
        t16 = (0, import_classnames.default)(t16, i18);
        continue;
      }
      if (e28.type === "string" || e28.type === "enum | string") {
        const i18 = e28.type === "string" ? [] : e28.values, [d10, y7] = R({ className: e28.className, customProperties: e28.customProperties, propValues: i18, parseValue: e28.parseValue, value: s20 });
        l10 = l2(l10, y7), t16 = (0, import_classnames.default)(t16, d10);
        continue;
      }
      if (e28.type === "boolean" && s20) {
        t16 = (0, import_classnames.default)(t16, e28.className);
        continue;
      }
    }
  }
  return a22.className = (0, import_classnames.default)(t16, r24.className), a22.style = l2(l10, r24.style), a22;
}

// node_modules/@radix-ui/themes/dist/esm/components/heading.js
var r3 = o5.forwardRef((p26, t16) => {
  const { children: e28, className: s20, asChild: a22, as: n20 = "h1", color: i18, ...m13 } = v(p26, n, r);
  return o5.createElement(dist_exports.Root, { "data-accent-color": i18, ...m13, ref: t16, className: (0, import_classnames2.default)("rt-Heading", s20) }, a22 ? e28 : o5.createElement(n20, null, e28));
});
r3.displayName = "Heading";

// node_modules/@radix-ui/themes/dist/esm/components/text.js
var o6 = __toESM(require_react(), 1);
var import_classnames3 = __toESM(require_classnames(), 1);
var p14 = o6.forwardRef((t16, r24) => {
  const { children: e28, className: s20, asChild: m13, as: a22 = "span", color: n20, ...P9 } = v(t16, n2, r);
  return o6.createElement(dist_exports.Root, { "data-accent-color": n20, ...P9, ref: r24, className: (0, import_classnames3.default)("rt-Text", s20) }, m13 ? e28 : o6.createElement(a22, null, e28));
});
p14.displayName = "Text";
...
var P = e6.createContext(void 0);
function H() {
  const a22 = e6.useContext(P);
  if (a22 === void 0) throw new Error("`useThemeContext` must be used within a `Theme`");
  return a22;
}
var R2 = e6.forwardRef((a22, s20) => e6.useContext(P) === void 0 ? e6.createElement(dist_exports33.Provider, { delayDuration: 200 }, e6.createElement(dist_exports5.Provider, { dir: "ltr" }, e6.createElement(I, { ...a22, ref: s20 }))) : e6.createElement(A, { ...a22, ref: s20 }));
R2.displayName = "Theme";
var I = e6.forwardRef((a22, s20) => {
  const { appearance: r24 = s2.appearance.default, accentColor: c4 = s2.accentColor.default, grayColor: l10 = s2.grayColor.default, panelBackground: p26 = s2.panelBackground.default, radius: n20 = s2.radius.default, scaling: t16 = s2.scaling.default, hasBackground: i18 = s2.hasBackground.default, ...u9 } = a22, [h4, m13] = e6.useState(r24);
  e6.useEffect(() => m13(r24), [r24]);
  const [y7, g8] = e6.useState(c4);
  e6.useEffect(() => g8(c4), [c4]);
  const [v11, C7] = e6.useState(l10);
  e6.useEffect(() => C7(l10), [l10]);
  const [k3, f18] = e6.useState(p26);
  e6.useEffect(() => f18(p26), [p26]);
  const [B5, x4] = e6.useState(n20);
  e6.useEffect(() => x4(n20), [n20]);
  const [T6, b6] = e6.useState(t16);
  return e6.useEffect(() => b6(t16), [t16]), e6.createElement(A, { ...u9, ref: s20, isRoot: true, hasBackground: i18, appearance: h4, accentColor: y7, grayColor: v11, panelBackground: k3, radius: B5, scaling: T6, onAppearanceChange: m13, onAccentColorChange: g8, onGrayColorChange: C7, onPanelBackgroundChange: f18, onRadiusChange: x4, onScalingChange: b6 });
});
I.displayName = "ThemeRoot";
var A = e6.forwardRef((a22, s20) => {
  const r24 = e6.useContext(P), { asChild: c4, isRoot: l10, hasBackground: p26, appearance: n20 = r24?.appearance ?? s2.appearance.default, accentColor: t16 = r24?.accentColor ?? s2.accentColor.default, grayColor: i18 = r24?.resolvedGrayColor ?? s2.grayColor.default, panelBackground: u9 = r24?.panelBackground ?? s2.panelBackground.default, radius: h4 = r24?.radius ?? s2.radius.default, scaling: m13 = r24?.scaling ?? s2.scaling.default, onAppearanceChange: y7 = d2, onAccentColorChange: g8 = d2, onGrayColorChange: v11 = d2, onPanelBackgroundChange: C7 = d2, onRadiusChange: k3 = d2, onScalingChange: f18 = d2, ...B5 } = a22, x4 = c4 ? dist_exports.Root : "div", T6 = i18 === "auto" ? a9(t16) : i18, b6 = a22.appearance === "light" || a22.appearance === "dark", S5 = p26 === void 0 ? l10 || b6 : p26;
  return e6.createElement(P.Provider, { value: e6.useMemo(() => ({ appearance: n20, accentColor: t16, grayColor: i18, resolvedGrayColor: T6, panelBackground: u9, radius: h4, scaling: m13, onAppearanceChange: y7, onAccentColorChange: g8, onGrayColorChange: v11, onPanelBackgroundChange: C7, onRadiusChange: k3, onScalingChange: f18 }), [n20, t16, i18, T6, u9, h4, m13, y7, g8, v11, C7, k3, f18]) }, e6.createElement(x4, { "data-is-root-theme": l10 ? "true" : "false", "data-accent-color": t16, "data-gray-color": T6, "data-has-background": S5 ? "true" : "false", "data-panel-background": u9, "data-radius": h4, "data-scaling": m13, ref: s20, ...B5, className: (0, import_classnames4.default)("radix-themes", { light: n20 === "light", dark: n20 === "dark" }, B5.className) }));
});
...
var a10 = (t16) => {
  if (!o7.isValidElement(t16)) throw Error(`Expected a single React Element child, but got: ${o7.Children.toArray(t16).map((e28) => typeof e28 == "object" && "type" in e28 && typeof e28.type == "string" ? e28.type : typeof e28).join(", ")}`);
  return t16;
};

// node_modules/@radix-ui/themes/dist/esm/components/alert-dialog.js
var n7 = (t16) => e7.createElement(dist_exports9.Root, { ...t16 });
n7.displayName = "AlertDialog.Root";
var s6 = e7.forwardRef(({ children: t16, ...o42 }, i18) => e7.createElement(dist_exports9.Trigger, { ...o42, ref: i18, asChild: true }, a10(t16)));
s6.displayName = "AlertDialog.Trigger";
var p15 = e7.forwardRef(({ align: t16, ...o42 }, i18) => {
  const { align: P9, ...c4 } = s, { className: f18 } = v({ align: t16 }, { align: P9 }), { className: C7, forceMount: d10, container: y7, ...v11 } = v(o42, c4);
  return e7.createElement(dist_exports9.Portal, { container: y7, forceMount: d10 }, e7.createElement(R2, { asChild: true }, e7.createElement(dist_exports9.Overlay, { className: "rt-BaseDialogOverlay rt-AlertDialogOverlay" }, e7.createElement("div", { className: "rt-BaseDialogScroll rt-AlertDialogScroll" }, e7.createElement("div", { className: `rt-BaseDialogScrollPadding rt-AlertDialogScrollPadding ${f18}` }, e7.createElement(dist_exports9.Content, { ...v11, ref: i18, className: (0, import_classnames5.default)("rt-BaseDialogContent", "rt-AlertDialogContent", C7) }))))));
});
p15.displayName = "AlertDialog.Content";
var g2 = e7.forwardRef((t16, o42) => e7.createElement(dist_exports9.Title, { asChild: true }, e7.createElement(r3, { size: "5", mb: "3", trim: "start", ...t16, asChild: false, ref: o42 })));
g2.displayName = "AlertDialog.Title";
var m4 = e7.forwardRef((t16, o42) => e7.createElement(dist_exports9.Description, { asChild: true }, e7.createElement(p14, { as: "p", size: "3", ...t16, asChild: false, ref: o42 })));
m4.displayName = "AlertDialog.Description";
var D = e7.forwardRef(({ children: t16, ...o42 }, i18) => e7.createElement(dist_exports9.Action, { ...o42, ref: i18, asChild: true }, a10(t16)));
D.displayName = "AlertDialog.Action";
var A2 = e7.forwardRef(({ children: t16, ...o42 }, i18) => e7.createElement(dist_exports9.Cancel, { ...o42, ref: i18, asChild: true }, a10(t16)));
A2.displayName = "AlertDialog.Cancel";

// node_modules/@radix-ui/themes/dist/esm/components/aspect-ratio.js
var i6 = dist_exports10.Root;
...
function d3(i18, e28) {
  const { asChild: r24, children: c4 } = i18;
  if (!r24) return typeof e28 == "function" ? e28(c4) : e28;
  const t16 = a11.Children.only(c4);
  return a11.cloneElement(t16, { children: typeof e28 == "function" ? e28(t16.props.children) : e28 });
}

// node_modules/@radix-ui/themes/dist/esm/components/avatar.js
var i7 = r4.forwardRef((t16, e28) => {
  const { asChild: o42, children: a22, className: s20, style: p26, color: v11, radius: f18, ...d10 } = v(t16, p, r);
  return r4.createElement(dist_exports11.Root, { "data-accent-color": v11, "data-radius": f18, className: (0, import_classnames6.default)("rt-reset", "rt-AvatarRoot", s20), style: p26, asChild: o42 }, d3({ asChild: o42, children: a22 }, r4.createElement(n8, { ref: e28, ...d10 })));
});
i7.displayName = "Avatar";
var n8 = r4.forwardRef(({ fallback: t16, ...e28 }, o42) => {
  const [a22, s20] = r4.useState("idle");
  return r4.createElement(r4.Fragment, null, a22 === "idle" || a22 === "loading" ? r4.createElement("span", { className: "rt-AvatarFallback" }) : null, a22 === "error" ? r4.createElement(dist_exports11.Fallback, { className: (0, import_classnames6.default)("rt-AvatarFallback", { "rt-one-letter": typeof t16 == "string" && t16.length === 1, "rt-two-letters": typeof t16 == "string" && t16.length === 2 }), delayMs: 0 }, t16) : null, r4.createElement(dist_exports11.Image, { ref: o42, className: "rt-AvatarImage", ...e28, onLoadingStatusChange: (p26) => {
    e28.onLoadingStatusChange?.(p26), s20(p26);
  } }));
});
n8.displayName = "AvatarImpl";

// node_modules/@radix-ui/themes/dist/esm/components/badge.js
var o8 = __toESM(require_react(), 1);
var import_classnames7 = __toESM(require_classnames(), 1);
var e8 = o8.forwardRef((r24, p26) => {
  const { asChild: t16, className: s20, color: a22, radius: m13, ...n20 } = v(r24, p2, r), d10 = t16 ? dist_exports.Root : "span";
  return o8.createElement(d10, { "data-accent-color": a22, "data-radius": m13, ...n20, ref: p26, className: (0, import_classnames7.default)("rt-reset", "rt-Badge", s20) });
});
e8.displayName = "Badge";

// node_modules/@radix-ui/themes/dist/esm/components/blockquote.js
var o9 = __toESM(require_react(), 1);
var import_classnames8 = __toESM(require_classnames(), 1);
var e9 = o9.forwardRef((t16, p26) => {
  const { asChild: r24, children: s20, className: m13, ...l10 } = t16, c4 = r24 ? dist_exports.Root : "blockquote";
  return o9.createElement(p14, { asChild: true, ...l10, ref: p26, className: (0, import_classnames8.default)("rt-Blockquote", m13) }, o9.createElement(c4, null, s20));
});
e9.displayName = "Blockquote";
...
var e10 = dist_exports.Root;
var r5 = dist_exports.Slottable;

// node_modules/@radix-ui/themes/dist/esm/components/box.js
var p16 = o10.forwardRef((r24, s20) => {
  const { className: t16, asChild: e28, as: m13 = "div", ...a22 } = v(r24, p3, u, r);
  return o10.createElement(e28 ? e10 : m13, { ...a22, ref: s20, className: (0, import_classnames9.default)("rt-Box", t16) });
});
p16.displayName = "Box";
...
var p17 = o11.forwardRef((r24, e28) => {
  const { className: s20, asChild: t16, as: m13 = "div", ...l10 } = v(r24, u2, u, r);
  return o11.createElement(t16 ? e10 : m13, { ...l10, ref: e28, className: (0, import_classnames10.default)("rt-Flex", s20) });
});
p17.displayName = "Flex";

// node_modules/@radix-ui/themes/dist/esm/components/spinner.js
var n11 = __toESM(require_react(), 1);
var import_classnames11 = __toESM(require_classnames(), 1);
var s7 = n11.forwardRef((i18, o42) => {
  const { className: a22, children: e28, loading: t16, ...m13 } = v(i18, s3, r);
  if (!t16) return e28;
  const r24 = n11.createElement("span", { ...m13, ref: o42, className: (0, import_classnames11.default)("rt-Spinner", a22) }, n11.createElement("span", { className: "rt-SpinnerLeaf" }), n11.createElement("span", { className: "rt-SpinnerLeaf" }), n11.createElement("span", { className: "rt-SpinnerLeaf" }), n11.createElement("span", { className: "rt-SpinnerLeaf" }), n11.createElement("span", { className: "rt-SpinnerLeaf" }), n11.createElement("span", { className: "rt-SpinnerLeaf" }), n11.createElement("span", { className: "rt-SpinnerLeaf" }), n11.createElement("span", { className: "rt-SpinnerLeaf" }));
  return e28 === void 0 ? r24 : n11.createElement(p17, { asChild: true, position: "relative", align: "center", justify: "center" }, n11.createElement("span", null, n11.createElement("span", { "aria-hidden": true, style: { display: "contents", visibility: "hidden" }, inert: void 0 }, e28), n11.createElement(p17, { asChild: true, align: "center", justify: "center", position: "absolute", inset: "0" }, n11.createElement("span", null, r24))));
});
s7.displayName = "Spinner";

// node_modules/@radix-ui/themes/dist/esm/components/visually-hidden.js
var d4 = dist_exports2.Root;
var e11 = dist_exports2.Root;

// node_modules/@radix-ui/themes/dist/esm/helpers/map-prop-values.js
function s8(e28, t16) {
  if (e28 !== void 0) return typeof e28 == "string" ? t16(e28) : Object.fromEntries(Object.entries(e28).map(([n20, o42]) => [n20, t16(o42)]));
}
function p18(e28) {
  return e28 === "3" ? "3" : "2";
}
...
// node_modules/@radix-ui/themes/dist/esm/components/_internal/base-button.js
var n12 = o12.forwardRef((t16, p26) => {
  const { size: i18 = i.size.default } = t16, { className: a22, children: e28, asChild: m13, color: d10, radius: l10, disabled: s20 = t16.loading, ...u9 } = v(t16, i, r), f18 = m13 ? dist_exports.Root : "button";
  return o12.createElement(f18, { "data-disabled": s20 || void 0, "data-accent-color": d10, "data-radius": l10, ...u9, ref: p26, className: (0, import_classnames12.default)("rt-reset", "rt-BaseButton", a22), disabled: s20 }, t16.loading ? o12.createElement(o12.Fragment, null, o12.createElement("span", { style: { display: "contents", visibility: "hidden" }, "aria-hidden": true }, e28), o12.createElement(d4, null, e28), o12.createElement(p17, { asChild: true, align: "center", justify: "center", position: "absolute", inset: "0" }, o12.createElement("span", null, o12.createElement(s7, { size: s8(i18, r6) })))) : e28);
});
n12.displayName = "BaseButton";

// node_modules/@radix-ui/themes/dist/esm/components/button.js
var o13 = t8.forwardRef(({ className: e28, ...n20 }, r24) => t8.createElement(n12, { ...n20, ref: r24, className: (0, import_classnames13.default)("rt-Button", e28) }));
o13.displayName = "Button";
...
var a13 = o14.createContext({});
var n13 = o14.forwardRef((t16, l10) => {
  const { size: e28 = a.size.default } = t16, { asChild: r24, children: C7, className: i18, color: c4, ...f18 } = v(t16, a, r), P9 = r24 ? dist_exports.Root : "div";
  return o14.createElement(P9, { "data-accent-color": c4, ...f18, className: (0, import_classnames14.default)("rt-CalloutRoot", i18), ref: l10 }, o14.createElement(a13.Provider, { value: o14.useMemo(() => ({ size: e28 }), [e28]) }, C7));
});
n13.displayName = "Callout.Root";
var m5 = o14.forwardRef(({ className: t16, ...l10 }, e28) => o14.createElement("div", { ...l10, className: (0, import_classnames14.default)("rt-CalloutIcon", t16), ref: e28 }));
m5.displayName = "Callout.Icon";
var u5 = o14.forwardRef(({ className: t16, ...l10 }, e28) => {
  const { size: r24 } = o14.useContext(a13);
  return o14.createElement(p14, { as: "p", size: s8(r24, p18), ...l10, asChild: false, ref: e28, className: (0, import_classnames14.default)("rt-CalloutText", t16) });
});
u5.displayName = "Callout.Text";
...
var o15 = r7.forwardRef((p26, e28) => {
  const { asChild: t16, className: s20, ...a22 } = v(p26, a2, r), m13 = t16 ? dist_exports.Root : "div";
  return r7.createElement(m13, { ref: e28, ...a22, className: (0, import_classnames15.default)("rt-reset", "rt-BaseCard", "rt-Card", s20) });
});
o15.displayName = "Card";
...
var [w, T2] = M(P2);
var v2 = e12.forwardRef((r24, n20) => {
  const { __scopeCheckboxGroup: o42, name: i18, defaultValue: c4, value: t16, required: p26 = false, disabled: s20 = false, orientation: h4, dir: l10, loop: b6 = true, onValueChange: u9, ...k3 } = r24, S5 = I2(o42), R6 = A3(l10), [E5 = [], a22] = useControllableState({ prop: t16, defaultProp: c4, onChange: u9 }), F = e12.useCallback((x4) => a22((m13 = []) => [...m13, x4]), [a22]), _ = e12.useCallback((x4) => a22((m13 = []) => m13.filter((V) => V !== x4)), [a22]);
  return e12.createElement(w, { scope: o42, name: i18, required: p26, disabled: s20, value: E5, onItemCheck: F, onItemUncheck: _ }, e12.createElement(dist_exports14.Root, { asChild: true, ...S5, orientation: h4, dir: R6, loop: b6 }, e12.createElement(Primitive2.div, { role: "group", "data-disabled": s20 ? "" : void 0, dir: R6, ...k3, ref: n20 })));
});
v2.displayName = P2;
var y = "CheckboxGroupItem";
var f6 = e12.forwardRef((r24, n20) => {
  const { __scopeCheckboxGroup: o42, disabled: i18, ...c4 } = r24, t16 = T2(y, o42), p26 = t16.disabled || i18, s20 = I2(o42), h4 = g3(o42), l10 = e12.useRef(null), b6 = useComposedRefs(n20, l10), u9 = t16.value?.includes(c4.value);
  return e12.createElement(dist_exports14.Item, { asChild: true, ...s20, focusable: !p26, active: u9 }, e12.createElement(dist_exports12.Root, { name: t16.name, disabled: p26, required: t16.required, checked: u9, ...h4, ...c4, ref: b6, onCheckedChange: (k3) => {
    k3 ? t16.onItemCheck(r24.value) : t16.onItemUncheck(r24.value);
  } }));
});
f6.displayName = y;
var B2 = "CheckboxGroupIndicator";
var G = e12.forwardRef((r24, n20) => {
  const { __scopeCheckboxGroup: o42, ...i18 } = r24, c4 = g3(o42);
  return e12.createElement(dist_exports12.Indicator, { ...c4, ...i18, ref: n20 });
});
G.displayName = B2;
var H2 = v2;
var K = f6;
...
var o16 = r8.forwardRef((p26, s20) => {
  const { className: t16, asChild: e28, as: i18 = "div", ...m13 } = v(p26, s4, u, r);
  return r8.createElement(e28 ? e10 : i18, { ...m13, ref: s20, className: (0, import_classnames16.default)("rt-Grid", t16) });
});
o16.displayName = "Grid";

// node_modules/@radix-ui/themes/dist/esm/components/icons.js
var import_react9 = __toESM(require_react(), 1);
var r9 = import_react9.default.forwardRef((e28, n20) => import_react9.default.createElement("svg", { width: "9", height: "9", viewBox: "0 0 9 9", fill: "currentcolor", xmlns: "http://www.w3.org/2000/svg", ...e28, ref: n20 }, import_react9.default.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", d: "M0.75 4.5C0.75 4.08579 1.08579 3.75 1.5 3.75H7.5C7.91421 3.75 8.25 4.08579 8.25 4.5C8.25 4.91421 7.91421 5.25 7.5 5.25H1.5C1.08579 5.25 0.75 4.91421 0.75 4.5Z" })));
r9.displayName = "ThickDividerHorizontalIcon";
var t9 = import_react9.default.forwardRef((e28, n20) => import_react9.default.createElement("svg", { width: "9", height: "9", viewBox: "0 0 9 9", fill: "currentcolor", xmlns: "http://www.w3.org/2000/svg", ...e28, ref: n20 }, import_react9.default.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", d: "M8.53547 0.62293C8.88226 0.849446 8.97976 1.3142 8.75325 1.66099L4.5083 8.1599C4.38833 8.34356 4.19397 8.4655 3.9764 8.49358C3.75883 8.52167 3.53987 8.45309 3.3772 8.30591L0.616113 5.80777C0.308959 5.52987 0.285246 5.05559 0.563148 4.74844C0.84105 4.44128 1.31533 4.41757 1.62249 4.69547L3.73256 6.60459L7.49741 0.840706C7.72393 0.493916 8.18868 0.396414 8.53547 0.62293Z" })));
t9.displayName = "ThickCheckIcon";
var i8 = import_react9.default.forwardRef((e28, n20) => import_react9.default.createElement("svg", { width: "9", height: "9", viewBox: "0 0 9 9", fill: "currentcolor", xmlns: "http://www.w3.org/2000/svg", ...e28, ref: n20 }, import_react9.default.createElement("path", { d: "M0.135232 3.15803C0.324102 2.95657 0.640521 2.94637 0.841971 3.13523L4.5 6.56464L8.158 3.13523C8.3595 2.94637 8.6759 2.95657 8.8648 3.15803C9.0536 3.35949 9.0434 3.67591 8.842 3.86477L4.84197 7.6148C4.64964 7.7951 4.35036 7.7951 4.15803 7.6148L0.158031 3.86477C-0.0434285 3.67591 -0.0536285 3.35949 0.135232 3.15803Z" })));
i8.displayName = "ChevronDownIcon";
var l5 = import_react9.default.forwardRef((e28, n20) => import_react9.default.createElement("svg", { width: "9", height: "9", viewBox: "0 0 9 9", fill: "currentcolor", xmlns: "http://www.w3.org/2000/svg", ...e28, ref: n20 }, import_react9.default.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", d: "M3.23826 0.201711C3.54108 -0.0809141 4.01567 -0.0645489 4.29829 0.238264L7.79829 3.98826C8.06724 4.27642 8.06724 4.72359 7.79829 5.01174L4.29829 8.76174C4.01567 9.06455 3.54108 9.08092 3.23826 8.79829C2.93545 8.51567 2.91909 8.04108 3.20171 7.73826L6.22409 4.5L3.20171 1.26174C2.91909 0.958928 2.93545 0.484337 3.23826 0.201711Z" })));
l5.displayName = "ThickChevronRightIcon";
...
var d6 = O();
var [v3, N2] = G2(x2);
var b = o18.forwardRef((e28, s20) => {
  const { __scopeCheckboxCards: r24, className: c4, color: p26, ...C7 } = v(e28, p4, r), t16 = d6(r24);
  return o18.createElement(v3, { scope: r24, size: e28.size, highContrast: e28.highContrast }, o18.createElement(o16, { asChild: true }, o18.createElement(H2, { ...t16, "data-accent-color": p26, ...C7, ref: s20, className: (0, import_classnames17.default)("rt-CheckboxCardsRoot", c4) })));
});
b.displayName = "CheckboxCards.Root";
var k = o18.forwardRef(({ __scopeCheckboxCards: e28, children: s20, className: r24, style: c4, ...p26 }, C7) => {
  const t16 = N2("CheckboxCardsItem", e28), i18 = d6(e28), { className: P9 } = v({ size: t16?.size, variant: "surface", highContrast: t16?.highContrast }, t2);
  return o18.createElement("label", { className: (0, import_classnames17.default)("rt-BaseCard", "rt-CheckboxCardsItem", r24), style: c4 }, s20, o18.createElement(K, { ...i18, ...p26, ref: C7, className: (0, import_classnames17.default)("rt-reset", "rt-BaseCheckboxRoot", "rt-CheckboxCardCheckbox", P9) }, o18.createElement(X, { ...i18, asChild: true, className: "rt-BaseCheckboxIndicator" }, o18.createElement(t9, null))));
});
...
var G3 = O();
var [E2, P3] = N3(b2);
var f7 = o19.forwardRef(({ color: s20 = e3.color.default, highContrast: e28 = e3.highContrast.default, size: r24 = e3.size.default, variant: t16 = e3.variant.default, ...p26 }, m13) => {
  const { __scopeCheckboxGroup: c4, className: h4, ...I6 } = v(p26, r), d10 = G3(c4);
  return o19.createElement(E2, { scope: c4, color: s20, size: r24, highContrast: e28, variant: t16 }, o19.createElement(H2, { ...d10, ...I6, ref: m13, className: (0, import_classnames18.default)("rt-CheckboxGroupRoot", h4) }));
});
f7.displayName = "CheckboxGroup.Root";
var l6 = o19.forwardRef((s20, e28) => {
  const { __scopeCheckboxGroup: r24, children: t16, className: p26, style: m13, ...c4 } = s20, { size: h4 } = P3("CheckboxGroupItem", r24);
  return t16 ? o19.createElement(p14, { as: "label", size: h4, className: (0, import_classnames18.default)("rt-CheckboxGroupItem", p26), style: m13 }, o19.createElement(i9, { __scopeCheckboxGroup: r24, ...c4, ref: e28 }), t16 && o19.createElement("span", { className: "rt-CheckboxGroupItemInner" }, t16)) : o19.createElement(i9, { __scopeCheckboxGroup: r24, ...c4, ref: e28, className: p26, style: m13 });
});
l6.displayName = "CheckboxGroup.Item";
var i9 = o19.forwardRef(({ __scopeCheckboxGroup: s20, ...e28 }, r24) => {
  const t16 = P3("CheckboxGroupItemCheckbox", s20), p26 = G3(s20), { color: m13, className: c4 } = v({ ...e28, ...t16 }, e3, r);
  return o19.createElement(K, { ...p26, "data-accent-color": m13, ...e28, ref: r24, className: (0, import_classnames18.default)("rt-reset", "rt-BaseCheckboxRoot", "rt-CheckboxGroupItemCheckbox", c4) }, o19.createElement(X, { ...p26, asChild: true, className: "rt-BaseCheckboxIndicator" }, o19.createElement(t9, null)));
});
i9.displayName = "CheckboxGroup.ItemCheckbox";
...
var c = e13.forwardRef((p26, i18) => {
  const { className: s20, color: h4, checked: a22, defaultChecked: o42, onCheckedChange: m13, ...n20 } = v(p26, t2, r), [r24, k3] = useControllableState({ prop: a22, defaultProp: o42, onChange: m13 });
  return e13.createElement(dist_exports12.Root, { "data-accent-color": h4, ...n20, defaultChecked: o42, checked: r24, onCheckedChange: k3, asChild: false, ref: i18, className: (0, import_classnames19.default)("rt-reset", "rt-BaseCheckboxRoot", "rt-CheckboxRoot", s20) }, e13.createElement(dist_exports12.Indicator, { asChild: true, className: "rt-BaseCheckboxIndicator rt-CheckboxIndicator" }, e13.createElement(r24 === "indeterminate" ? r9 : t9, null)));
});
...
var p19 = r10.forwardRef((o42, t16) => {
  const { asChild: s20, className: m13, color: e28, ...d10 } = v(o42, f, r), n20 = o42.variant === "ghost" ? e28 || void 0 : e28, a22 = s20 ? dist_exports.Root : "code";
  return r10.createElement(a22, { "data-accent-color": n20, ...d10, ref: t16, className: (0, import_classnames20.default)("rt-reset", "rt-Code", m13) });
});
p19.displayName = "Code";
...
var p20 = o20.forwardRef(({ width: n20, minWidth: s20, maxWidth: i18, height: m13, minHeight: a22, maxHeight: f18, ...P9 }, l10) => {
  const { asChild: r24, children: C7, className: c4, ...y7 } = v(P9, n3, u, r), { className: d10, style: h4 } = v({ width: n20, minWidth: s20, maxWidth: i18, height: m13, minHeight: a22, maxHeight: f18 }, t, e), u9 = r24 ? dist_exports.Root : "div";
  return o20.createElement(u9, { ...y7, ref: l10, className: (0, import_classnames21.default)("rt-Container", c4) }, d3({ asChild: r24, children: C7 }, (v11) => o20.createElement("div", { className: (0, import_classnames21.default)("rt-ContainerInner", d10), style: h4 }, v11)));
});
...
function a16(r24) {
  const { m: t16, mx: m13, my: o42, mt: p26, mr: n20, mb: s20, ml: e28, ...i18 } = r24;
  return { m: t16, mx: m13, my: o42, mt: p26, mr: n20, mb: s20, ml: e28, rest: i18 };
}
...
var r11 = r.m.values;
function S(s20) {
  const [e28, t16] = R({ className: "rt-r-m", customProperties: ["--margin"], propValues: r11, value: s20.m }), [a22, o42] = R({ className: "rt-r-mx", customProperties: ["--margin-left", "--margin-right"], propValues: r11, value: s20.mx }), [l10, i18] = R({ className: "rt-r-my", customProperties: ["--margin-top", "--margin-bottom"], propValues: r11, value: s20.my }), [p26, u9] = R({ className: "rt-r-mt", customProperties: ["--margin-top"], propValues: r11, value: s20.mt }), [n20, c4] = R({ className: "rt-r-mr", customProperties: ["--margin-right"], propValues: r11, value: s20.mr }), [g8, P9] = R({ className: "rt-r-mb", customProperties: ["--margin-bottom"], propValues: r11, value: s20.mb }), [N8, C7] = R({ className: "rt-r-ml", customProperties: ["--margin-left"], propValues: r11, value: s20.ml });
  return [(0, import_classnames22.default)(e28, a22, l10, p26, n20, g8, N8), l2(t16, o42, i18, u9, c4, P9, C7)];
}

// node_modules/@radix-ui/themes/dist/esm/components/scroll-area.js
var c2 = r12.forwardRef((n20, S5) => {
  const { rest: f18, ...P9 } = a16(n20), [u9, A4] = S(P9), { asChild: a22, children: d10, className: y7, style: v11, type: t16, scrollHideDelay: N8 = t16 !== "scroll" ? 0 : void 0, dir: V, size: i18 = t3.size.default, radius: p26 = t3.radius.default, scrollbars: l10 = t3.scrollbars.default, ...b6 } = f18;
  return r12.createElement(dist_exports25.Root, { type: t16, scrollHideDelay: N8, className: (0, import_classnames23.default)("rt-ScrollAreaRoot", u9, y7), style: l2(A4, v11), asChild: a22 }, d3({ asChild: a22, children: d10 }, (g8) => r12.createElement(r12.Fragment, null, r12.createElement(dist_exports25.Viewport, { ...b6, ref: S5, className: "rt-ScrollAreaViewport" }, g8), r12.createElement("div", { className: "rt-ScrollAreaViewportFocusRing" }), l10 !== "vertical" ? r12.createElement(dist_exports25.Scrollbar, { "data-radius": p26, orientation: "horizontal", className: (0, import_classnames23.default)("rt-ScrollAreaScrollbar", g({ className: "rt-r-size", value: i18, propValues: t3.size.values })) }, r12.createElement(dist_exports25.Thumb, { className: "rt-ScrollAreaThumb" })) : null, l10 !== "horizontal" ? r12.createElement(dist_exports25.Scrollbar, { "data-radius": p26, orientation: "vertical", className: (0, import_classnames23.default)("rt-ScrollAreaScrollbar", g({ className: "rt-r-size", value: i18, propValues: t3.size.values })) }, r12.createElement(dist_exports25.Thumb, { className: "rt-ScrollAreaThumb" })) : null, l10 === "both" ? r12.createElement(dist_exports25.Corner, { className: "rt-ScrollAreaCorner" }) : null)));
});
c2.displayName = "ScrollArea";

// node_modules/@radix-ui/themes/dist/esm/components/context-menu.js
var h = (t16) => e14.createElement(dist_exports16.Root, { ...t16 });
h.displayName = "ContextMenu.Root";
var S2 = e14.forwardRef(({ children: t16, ...n20 }, o42) => e14.createElement(dist_exports16.Trigger, { ...n20, ref: o42, asChild: true }, a10(t16)));
S2.displayName = "ContextMenu.Trigger";
var g4 = e14.createContext({});
var v4 = e14.forwardRef((t16, n20) => {
  const o42 = H(), { size: a22 = n4.size.default, variant: u9 = n4.variant.default, highContrast: i18 = n4.highContrast.default } = t16, { className: C7, children: m13, color: x4, container: M2, forceMount: l10, ...L2 } = v(t16, n4), c4 = x4 || o42.accentColor;
  return e14.createElement(dist_exports16.Portal, { container: M2, forceMount: l10 }, e14.createElement(R2, { asChild: true }, e14.createElement(dist_exports16.Content, { "data-accent-color": c4, alignOffset: -Number(a22) * 4, collisionPadding: 10, ...L2, asChild: false, ref: n20, className: (0, import_classnames24.default)("rt-PopperContent", "rt-BaseMenuContent", "rt-ContextMenuContent", C7) }, e14.createElement(c2, { type: "auto" }, e14.createElement("div", { className: (0, import_classnames24.default)("rt-BaseMenuViewport", "rt-ContextMenuViewport") }, e14.createElement(g4.Provider, { value: e14.useMemo(() => ({ size: a22, variant: u9, color: c4, highContrast: i18 }), [a22, u9, c4, i18]) }, m13))))));
});
v4.displayName = "ContextMenu.Content";
var y2 = e14.forwardRef(({ className: t16, ...n20 }, o42) => e14.createElement(dist_exports16.Label, { ...n20, asChild: false, ref: o42, className: (0, import_classnames24.default)("rt-BaseMenuLabel", "rt-ContextMenuLabel", t16) }));
y2.displayName = "ContextMenu.Label";
var R3 = e14.forwardRef((t16, n20) => {
  const { className: o42, children: a22, color: u9 = a3.color.default, shortcut: i18, ...C7 } = t16;
  return e14.createElement(dist_exports16.Item, { "data-accent-color": u9, ...C7, ref: n20, className: (0, import_classnames24.default)("rt-reset", "rt-BaseMenuItem", "rt-ContextMenuItem", o42) }, e14.createElement(dist_exports.Slottable, null, a22), i18 && e14.createElement("div", { className: "rt-BaseMenuShortcut rt-ContextMenuShortcut" }, i18));
});
R3.displayName = "ContextMenu.Item";
var N4 = e14.forwardRef(({ className: t16, ...n20 }, o42) => e14.createElement(dist_exports16.Group, { ...n20, asChild: false, ref: o42, className: (0, import_classnames24.default)("rt-BaseMenuGroup", "rt-ContextMenuGroup", t16) }));
N4.displayName = "ContextMenu.Group";
var T3 = e14.forwardRef(({ className: t16, ...n20 }, o42) => e14.createElement(dist_exports16.RadioGroup, { ...n20, asChild: false, ref: o42, className: (0, import_classnames24.default)("rt-BaseMenuRadioGroup", "rt-ContextMenuRadioGroup", t16) }));
T3.displayName = "ContextMenu.RadioGroup";
var E3 = e14.forwardRef((t16, n20) => {
  const { children: o42, className: a22, color: u9 = p5.color.default, ...i18 } = t16;
  return e14.createElement(dist_exports16.RadioItem, { ...i18, asChild: false, ref: n20, "data-accent-color": u9, className: (0, import_classnames24.default)("rt-BaseMenuItem", "rt-BaseMenuRadioItem", "rt-ContextMenuItem", "rt-ContextMenuRadioItem", a22) }, e14.createElement(dist_exports.Slottable, null, o42), e14.createElement(dist_exports16.ItemIndicator, { className: "rt-BaseMenuItemIndicator rt-ContextMenuItemIndicator" }, e14.createElement(t9, { className: "rt-BaseMenuItemIndicatorIcon rt-ContextMenuItemIndicatorIcon" })));
});
E3.displayName = "ContextMenu.RadioItem";
var G4 = e14.forwardRef((t16, n20) => {
  const { children: o42, className: a22, shortcut: u9, color: i18 = i2.color.default, ...C7 } = t16;
  return e14.createElement(dist_exports16.CheckboxItem, { ...C7, asChild: false, ref: n20, "data-accent-color": i18, className: (0, import_classnames24.default)("rt-BaseMenuItem", "rt-BaseMenuCheckboxItem", "rt-ContextMenuItem", "rt-ContextMenuCheckboxItem", a22) }, e14.createElement(dist_exports.Slottable, null, o42), e14.createElement(dist_exports16.ItemIndicator, { className: "rt-BaseMenuItemIndicator rt-ContextMenuItemIndicator" }, e14.createElement(t9, { className: "rt-BaseMenuItemIndicatorIcon rt-ContextMenuItemIndicatorIcon" })), u9 && e14.createElement("div", { className: "rt-BaseMenuShortcut rt-ContextMenuShortcut" }, u9));
});
G4.displayName = "ContextMenu.CheckboxItem";
var w2 = (t16) => e14.createElement(dist_exports16.Sub, { ...t16 });
w2.displayName = "ContextMenu.Sub";
var B3 = e14.forwardRef((t16, n20) => {
  const { className: o42, children: a22, ...u9 } = t16;
  return e14.createElement(dist_exports16.SubTrigger, { ...u9, asChild: false, ref: n20, className: (0, import_classnames24.default)("rt-BaseMenuItem", "rt-BaseMenuSubTrigger", "rt-ContextMenuItem", "rt-ContextMenuSubTrigger", o42) }, a22, e14.createElement("div", { className: "rt-BaseMenuShortcut rt-ContextMenuShortcut" }, e14.createElement(l5, { className: "rt-BaseMenuSubTriggerIcon rt-ContextMenuSubTriggerIcon" })));
});
B3.displayName = "ContextMenu.SubTrigger";
var k2 = e14.forwardRef((t16, n20) => {
  const { size: o42, variant: a22, color: u9, highContrast: i18 } = e14.useContext(g4), { className: C7, children: m13, container: x4, forceMount: M2, ...l10 } = v({ size: o42, variant: a22, color: u9, highContrast: i18, ...t16 }, n4);
  return e14.createElement(dist_exports16.Portal, { container: x4, forceMount: M2 }, e14.createElement(R2, { asChild: true }, e14.createElement(dist_exports16.SubContent, { "data-accent-color": u9, alignOffset: -Number(o42) * 4, sideOffset: 1, collisionPadding: 10, ...l10, asChild: false, ref: n20, className: (0, import_classnames24.default)("rt-PopperContent", "rt-BaseMenuContent", "rt-BaseMenuSubContent", "rt-ContextMenuContent", "rt-ContextMenuSubContent", C7) }, e14.createElement(c2, { type: "auto" }, e14.createElement("div", { className: (0, import_classnames24.default)("rt-BaseMenuViewport", "rt-ContextMenuViewport") }, m13)))));
});
k2.displayName = "ContextMenu.SubContent";
var W = e14.forwardRef(({ className: t16, ...n20 }, o42) => e14.createElement(dist_exports16.Separator, { ...n20, asChild: false, ref: o42, className: (0, import_classnames24.default)("rt-BaseMenuSeparator", "rt-ContextMenuSeparator", t16) }));
...
var i10 = t11.forwardRef((a22, e28) => {
  const { className: s20, ...o42 } = v(a22, n5, r);
  return t11.createElement(p14, { asChild: true }, t11.createElement("dl", { ...o42, ref: e28, className: (0, import_classnames25.default)("rt-DataListRoot", s20) }));
});
i10.displayName = "DataList.Root";
var m6 = t11.forwardRef((a22, e28) => {
  const { className: s20, ...o42 } = v(a22, p6);
  return t11.createElement("div", { ...o42, ref: e28, className: (0, import_classnames25.default)("rt-DataListItem", s20) });
});
m6.displayName = "DataList.Item";
var L = t11.forwardRef((a22, e28) => {
  const { className: s20, color: o42, ...n20 } = v(a22, m);
  return t11.createElement("dt", { ...n20, "data-accent-color": o42, ref: e28, className: (0, import_classnames25.default)("rt-DataListLabel", s20) });
});
L.displayName = "DataList.Label";
var l7 = t11.forwardRef(({ children: a22, className: e28, ...s20 }, o42) => t11.createElement("dd", { ...s20, ref: o42, className: (0, import_classnames25.default)(e28, "rt-DataListValue") }, a22));
...
var s13 = (e28) => o21.createElement(dist_exports8.Root, { ...e28, modal: true });
s13.displayName = "Dialog.Root";
var n14 = o21.forwardRef(({ children: e28, ...i18 }, r24) => o21.createElement(dist_exports8.Trigger, { ...i18, ref: r24, asChild: true }, a10(e28)));
n14.displayName = "Dialog.Trigger";
var p21 = o21.forwardRef(({ align: e28, ...i18 }, r24) => {
  const { align: P9, ...f18 } = s, { className: C7 } = v({ align: e28 }, { align: P9 }), { className: d10, forceMount: c4, container: y7, ...T6 } = v(i18, f18);
  return o21.createElement(dist_exports8.Portal, { container: y7, forceMount: c4 }, o21.createElement(R2, { asChild: true }, o21.createElement(dist_exports8.Overlay, { className: "rt-BaseDialogOverlay rt-DialogOverlay" }, o21.createElement("div", { className: "rt-BaseDialogScroll rt-DialogScroll" }, o21.createElement("div", { className: `rt-BaseDialogScrollPadding rt-DialogScrollPadding ${C7}` }, o21.createElement(dist_exports8.Content, { ...T6, ref: r24, className: (0, import_classnames26.default)("rt-BaseDialogContent", "rt-DialogContent", d10) }))))));
});
p21.displayName = "Dialog.Content";
var g5 = o21.forwardRef((e28, i18) => o21.createElement(dist_exports8.Title, { asChild: true }, o21.createElement(r3, { size: "5", mb: "3", trim: "start", ...e28, asChild: false, ref: i18 })));
g5.displayName = "Dialog.Title";
var m7 = o21.forwardRef((e28, i18) => o21.createElement(dist_exports8.Description, { asChild: true }, o21.createElement(p14, { as: "p", size: "3", ...e28, asChild: false, ref: i18 })));
m7.displayName = "Dialog.Description";
var D2 = o21.forwardRef(({ children: e28, ...i18 }, r24) => o21.createElement(dist_exports8.Close, { ...i18, ref: r24, asChild: true }, a10(e28)));
...
var I3 = (e28) => o22.createElement(dist_exports18.Root, { ...e28 });
I3.displayName = "DropdownMenu.Root";
var h2 = o22.forwardRef(({ children: e28, ...n20 }, r24) => o22.createElement(dist_exports18.Trigger, { ...n20, ref: r24, asChild: true }, a10(e28)));
h2.displayName = "DropdownMenu.Trigger";
var b3 = o22.createContext({});
var g6 = o22.forwardRef((e28, n20) => {
  const r24 = H(), { size: s20 = n4.size.default, variant: p26 = n4.variant.default, highContrast: u9 = n4.highContrast.default } = e28, { className: a22, children: m13, color: M2, container: w3, forceMount: D3, ...k3 } = v(e28, n4), l10 = M2 || r24.accentColor;
  return o22.createElement(dist_exports18.Portal, { container: w3, forceMount: D3 }, o22.createElement(R2, { asChild: true }, o22.createElement(dist_exports18.Content, { "data-accent-color": l10, align: "start", sideOffset: 4, collisionPadding: 10, ...k3, asChild: false, ref: n20, className: (0, import_classnames27.default)("rt-PopperContent", "rt-BaseMenuContent", "rt-DropdownMenuContent", a22) }, o22.createElement(c2, { type: "auto" }, o22.createElement("div", { className: (0, import_classnames27.default)("rt-BaseMenuViewport", "rt-DropdownMenuViewport") }, o22.createElement(b3.Provider, { value: o22.useMemo(() => ({ size: s20, variant: p26, color: l10, highContrast: u9 }), [s20, p26, l10, u9]) }, m13))))));
});
g6.displayName = "DropdownMenu.Content";
var y3 = o22.forwardRef(({ className: e28, ...n20 }, r24) => o22.createElement(dist_exports18.Label, { ...n20, asChild: false, ref: r24, className: (0, import_classnames27.default)("rt-BaseMenuLabel", "rt-DropdownMenuLabel", e28) }));
y3.displayName = "DropdownMenu.Label";
var v6 = o22.forwardRef((e28, n20) => {
  const { className: r24, children: s20, color: p26 = a3.color.default, shortcut: u9, ...a22 } = e28;
  return o22.createElement(dist_exports18.Item, { "data-accent-color": p26, ...a22, ref: n20, className: (0, import_classnames27.default)("rt-reset", "rt-BaseMenuItem", "rt-DropdownMenuItem", r24) }, o22.createElement(dist_exports.Slottable, null, s20), u9 && o22.createElement("div", { className: "rt-BaseMenuShortcut rt-DropdownMenuShortcut" }, u9));
});
v6.displayName = "DropdownMenu.Item";
var R4 = o22.forwardRef(({ className: e28, ...n20 }, r24) => o22.createElement(dist_exports18.Group, { ...n20, asChild: false, ref: r24, className: (0, import_classnames27.default)("rt-BaseMenuGroup", "rt-DropdownMenuGroup", e28) }));
R4.displayName = "DropdownMenu.Group";
var S3 = o22.forwardRef(({ className: e28, ...n20 }, r24) => o22.createElement(dist_exports18.RadioGroup, { ...n20, asChild: false, ref: r24, className: (0, import_classnames27.default)("rt-BaseMenuRadioGroup", "rt-DropdownMenuRadioGroup", e28) }));
S3.displayName = "DropdownMenu.RadioGroup";
var x3 = o22.forwardRef((e28, n20) => {
  const { children: r24, className: s20, color: p26 = p5.color.default, ...u9 } = e28;
  return o22.createElement(dist_exports18.RadioItem, { ...u9, asChild: false, ref: n20, "data-accent-color": p26, className: (0, import_classnames27.default)("rt-BaseMenuItem", "rt-BaseMenuRadioItem", "rt-DropdownMenuItem", "rt-DropdownMenuRadioItem", s20) }, r24, o22.createElement(dist_exports18.ItemIndicator, { className: "rt-BaseMenuItemIndicator rt-DropdownMenuItemIndicator" }, o22.createElement(t9, { className: "rt-BaseMenuItemIndicatorIcon rt-DropdownMenuItemIndicatorIcon" })));
});
x3.displayName = "DropdownMenu.RadioItem";
var T4 = o22.forwardRef((e28, n20) => {
  const { children: r24, className: s20, shortcut: p26, color: u9 = i2.color.default, ...a22 } = e28;
  return o22.createElement(dist_exports18.CheckboxItem, { ...a22, asChild: false, ref: n20, "data-accent-color": u9, className: (0, import_classnames27.default)("rt-BaseMenuItem", "rt-BaseMenuCheckboxItem", "rt-DropdownMenuItem", "rt-DropdownMenuCheckboxItem", s20) }, r24, o22.createElement(dist_exports18.ItemIndicator, { className: "rt-BaseMenuItemIndicator rt-DropdownMenuItemIndicator" }, o22.createElement(t9, { className: "rt-BaseMenuItemIndicatorIcon rt-ContextMenuItemIndicatorIcon" })), p26 && o22.createElement("div", { className: "rt-BaseMenuShortcut rt-DropdownMenuShortcut" }, p26));
});
T4.displayName = "DropdownMenu.CheckboxItem";
var N5 = (e28) => o22.createElement(dist_exports18.Sub, { ...e28 });
N5.displayName = "DropdownMenu.Sub";
var E4 = o22.forwardRef((e28, n20) => {
  const { className: r24, children: s20, ...p26 } = e28;
  return o22.createElement(dist_exports18.SubTrigger, { ...p26, asChild: false, ref: n20, className: (0, import_classnames27.default)("rt-BaseMenuItem", "rt-BaseMenuSubTrigger", "rt-DropdownMenuItem", "rt-DropdownMenuSubTrigger", r24) }, s20, o22.createElement("div", { className: "rt-BaseMenuShortcut rt-DropdownMenuShortcut" }, o22.createElement(l5, { className: "rt-BaseMenuSubTriggerIcon rt-DropdownMenuSubtriggerIcon" })));
});
E4.displayName = "DropdownMenu.SubTrigger";
var G5 = o22.forwardRef((e28, n20) => {
  const { size: r24, variant: s20, color: p26, highContrast: u9 } = o22.useContext(b3), { className: a22, children: m13, container: M2, forceMount: w3, ...D3 } = v({ size: r24, variant: s20, color: p26, highContrast: u9, ...e28 }, n4);
  return o22.createElement(dist_exports18.Portal, { container: M2, forceMount: w3 }, o22.createElement(R2, { asChild: true }, o22.createElement(dist_exports18.SubContent, { "data-accent-color": p26, alignOffset: -Number(r24) * 4, sideOffset: 1, collisionPadding: 10, ...D3, asChild: false, ref: n20, className: (0, import_classnames27.default)("rt-PopperContent", "rt-BaseMenuContent", "rt-BaseMenuSubContent", "rt-DropdownMenuContent", "rt-DropdownMenuSubContent", a22) }, o22.createElement(c2, { type: "auto" }, o22.createElement("div", { className: (0, import_classnames27.default)("rt-BaseMenuViewport", "rt-DropdownMenuViewport") }, m13)))));
});
G5.displayName = "DropdownMenu.SubContent";
var B4 = o22.forwardRef(({ className: e28, ...n20 }, r24) => o22.createElement(dist_exports18.Separator, { ...n20, asChild: false, ref: r24, className: (0, import_classnames27.default)("rt-BaseMenuSeparator", "rt-DropdownMenuSeparator", e28) }));
...
var e15 = o23.forwardRef((p26, m13) => {
  const { asChild: r24, className: t16, ...s20 } = v(p26, t4), f18 = r24 ? dist_exports.Root : "em";
  return o23.createElement(f18, { ...s20, ref: m13, className: (0, import_classnames28.default)("rt-Em", t16) });
});
e15.displayName = "Em";
...
var p22 = (e28) => r14.createElement(dist_exports19.Root, { closeDelay: 150, openDelay: 200, ...e28 });
p22.displayName = "HoverCard.Root";
var s14 = r14.forwardRef(({ children: e28, className: t16, ...n20 }, a22) => r14.createElement(dist_exports19.Trigger, { ref: a22, className: (0, import_classnames29.default)("rt-HoverCardTrigger", t16), ...n20, asChild: true }, a10(e28)));
s14.displayName = "HoverCard.Trigger";
var C3 = r14.forwardRef((e28, t16) => {
  const { className: n20, forceMount: a22, container: m13, ...d10 } = v(e28, p7);
  return r14.createElement(dist_exports19.Portal, { container: m13, forceMount: a22 }, r14.createElement(R2, { asChild: true }, r14.createElement(dist_exports19.Content, { align: "start", sideOffset: 8, collisionPadding: 10, ...d10, ref: t16, className: (0, import_classnames29.default)("rt-PopperContent", "rt-HoverCardContent", n20) })));
});
...
var o24 = t12.forwardRef(({ className: e28, ...n20 }, r24) => t12.createElement(n12, { ...n20, ref: r24, className: (0, import_classnames30.default)("rt-IconButton", e28) }));
o24.displayName = "IconButton";
...
var e16 = o25.forwardRef((r24, t16) => {
  const { asChild: p26, className: s20, ...n20 } = v(r24, u3, r), m13 = p26 ? dist_exports.Root : "div";
  return o25.createElement(m13, { ...n20, ref: t16, className: (0, import_classnames31.default)("rt-Inset", s20) });
});
e16.displayName = "Inset";
...
var r15 = o26.forwardRef((p26, e28) => {
  const { asChild: t16, className: s20, ...m13 } = v(p26, o, r), d10 = t16 ? dist_exports.Root : "kbd";
  return o26.createElement(d10, { ...m13, ref: e28, className: (0, import_classnames32.default)("rt-reset", "rt-Kbd", s20) });
});
r15.displayName = "Kbd";
...
var e17 = o27.forwardRef((p26, t16) => {
  const { children: r24, className: s20, color: n20, asChild: i18, ...m13 } = v(p26, m2);
  return o27.createElement(p14, { ...m13, "data-accent-color": n20, ref: t16, asChild: true, className: (0, import_classnames33.default)("rt-reset", "rt-Link", s20) }, i18 ? r24 : o27.createElement("a", null, r24));
});
e17.displayName = "Link";
...
var P4 = (e28) => o28.createElement(dist_exports22.Root, { ...e28 });
P4.displayName = "Popover.Root";
var s16 = o28.forwardRef(({ children: e28, ...r24 }, t16) => o28.createElement(dist_exports22.Trigger, { ...r24, ref: t16, asChild: true }, a10(e28)));
s16.displayName = "Popover.Trigger";
var i13 = o28.forwardRef((e28, r24) => {
  const { className: t16, forceMount: a22, container: f18, ...l10 } = v(e28, r2);
  return o28.createElement(dist_exports22.Portal, { container: f18, forceMount: a22 }, o28.createElement(R2, { asChild: true }, o28.createElement(dist_exports22.Content, { align: "start", sideOffset: 8, collisionPadding: 10, ...l10, ref: r24, className: (0, import_classnames34.default)("rt-PopperContent", "rt-PopoverContent", t16) })));
});
i13.displayName = "Popover.Content";
var v7 = o28.forwardRef(({ children: e28, ...r24 }, t16) => o28.createElement(dist_exports22.Close, { ...r24, ref: t16, asChild: true }, a10(e28)));
v7.displayName = "Popover.Close";
var m8 = o28.forwardRef(({ children: e28, ...r24 }, t16) => o28.createElement(dist_exports22.Anchor, { ...r24, ref: t16 }));
m8.displayName = "Popover.Anchor";

// node_modules/@radix-ui/themes/dist/esm/components/portal.js
var r16 = dist_exports7.Root;
...
var s17 = o29.forwardRef((p26, t16) => {
  const { className: m13, style: i18, color: a22, radius: n20, duration: P9, ...r24 } = v(p26, a4, r);
  return o29.createElement(dist_exports23.Root, { "data-accent-color": a22, "data-radius": n20, ref: t16, className: (0, import_classnames35.default)("rt-ProgressRoot", m13), style: l2({ "--progress-duration": "value" in r24 ? void 0 : P9, "--progress-value": "value" in r24 ? r24.value : void 0, "--progress-max": "max" in r24 ? r24.max : void 0 }, i18), ...r24, asChild: false }, o29.createElement(dist_exports23.Indicator, { className: "rt-ProgressIndicator" }));
});
s17.displayName = "Progress";
...
var e18 = o30.forwardRef((t16, p26) => {
  const { asChild: r24, className: s20, ...m13 } = v(t16, p8), f18 = r24 ? dist_exports.Root : "q";
  return o30.createElement(f18, { ...m13, ref: p26, className: (0, import_classnames36.default)("rt-Quote", s20) });
});
e18.displayName = "Quote";
...
var p23 = o31.forwardRef((r24, t16) => {
  const { className: e28, color: d10, ...m13 } = v(r24, p9, r);
  return o31.createElement(o16, { asChild: true }, o31.createElement(dist_exports24.Root, { "data-accent-color": d10, ...m13, ref: t16, className: (0, import_classnames37.default)("rt-RadioCardsRoot", e28) }));
});
p23.displayName = "RadioCards.Root";
var i14 = o31.forwardRef(({ className: r24, ...t16 }, e28) => o31.createElement(dist_exports24.Item, { ...t16, asChild: false, ref: e28, className: (0, import_classnames37.default)("rt-reset", "rt-BaseCard", "rt-RadioCardsItem", r24) }));
...
var f12 = "RadioGroup";
var [v8] = dist_exports4.createContextScope(f12, [dist_exports24.createRadioGroupScope]);
var P6 = dist_exports24.createRadioGroupScope();
var [S4, l8] = v8(f12);
var I4 = o32.forwardRef(({ color: a22 = a5.color.default, highContrast: e28 = a5.highContrast.default, size: r24 = a5.size.default, variant: t16 = a5.variant.default, ...i18 }, s20) => {
  const { __scopeRadioGroup: p26, className: m13, ...x4 } = v(i18, r), C7 = P6(p26);
  return o32.createElement(S4, { scope: p26, color: a22, highContrast: e28, size: r24, variant: t16 }, o32.createElement(dist_exports24.Root, { ...C7, ...x4, ref: s20, className: (0, import_classnames38.default)("rt-RadioGroupRoot", m13) }));
});
I4.displayName = "RadioGroup.Root";
var y4 = o32.forwardRef((a22, e28) => {
  const { __scopeRadioGroup: r24, children: t16, className: i18, style: s20, ...p26 } = a22, { size: m13 } = l8("RadioGroupItem", r24);
  return t16 ? o32.createElement(p14, { as: "label", size: m13, className: (0, import_classnames38.default)("rt-RadioGroupItem", i18), style: s20 }, o32.createElement(n17, { __scopeRadioGroup: r24, ...p26, ref: e28 }), t16 && o32.createElement("span", { className: "rt-RadioGroupItemInner" }, t16)) : o32.createElement(n17, { __scopeRadioGroup: r24, ...p26, ref: e28, className: i18, style: s20 });
});
y4.displayName = "RadioGroup.Item";
var n17 = o32.forwardRef(({ __scopeRadioGroup: a22, ...e28 }, r24) => {
  const t16 = l8("RadioGroupItemRadio", a22), i18 = P6(a22), { color: s20, className: p26 } = v({ ...e28, ...t16 }, a5, r);
  return o32.createElement(dist_exports24.Item, { ...i18, "data-accent-color": s20, ...e28, asChild: false, ref: r24, className: (0, import_classnames38.default)("rt-reset", "rt-BaseRadioRoot", p26) });
});
n17.displayName = "RadioGroup.ItemRadio";
...
var e19 = o33.forwardRef((r24, t16) => {
  const p26 = o33.useRef(null), { className: a22, color: s20, onChange: i18, onValueChange: n20, ...m13 } = v(r24, t5, r);
  return o33.createElement("input", { type: "radio", "data-accent-color": s20, ...m13, onChange: composeEventHandlers(i18, (d10) => n20?.(d10.currentTarget.value)), ref: composeRefs(p26, t16), className: (0, import_classnames39.default)("rt-reset", "rt-BaseRadioRoot", "rt-RadioRoot", a22) });
});
e19.displayName = "Radio";
...
var o34 = e20.forwardRef(({ className: t16, children: r24, ...s20 }, p26) => e20.createElement(dist_exports.Root, { ...s20, ref: p26, className: (0, import_classnames40.default)("rt-reset", t16) }, a10(r24)));
o34.displayName = "Reset";
...
var p24 = e21.forwardRef((o42, t16) => {
  const { className: r24, children: n20, radius: d10, value: i18, defaultValue: g8, onValueChange: f18, ...P9 } = v(o42, o2, r), [u9, C7] = useControllableState({ prop: i18, onChange: f18, defaultProp: g8 });
  return e21.createElement(dist_exports31.Root, { "data-disabled": o42.disabled || void 0, "data-radius": d10, ref: t16, className: (0, import_classnames41.default)("rt-SegmentedControlRoot", r24), onValueChange: (s20) => {
    s20 && C7(s20);
  }, ...P9, type: "single", value: u9, asChild: false, disabled: !!o42.disabled }, n20, e21.createElement("div", { className: "rt-SegmentedControlIndicator" }));
});
p24.displayName = "SegmentedControl.Root";
var l9 = e21.forwardRef(({ children: o42, className: t16, ...r24 }, n20) => e21.createElement(dist_exports31.Item, { ref: n20, className: (0, import_classnames41.default)("rt-reset", "rt-SegmentedControlItem", t16), ...r24, disabled: false, asChild: false }, e21.createElement("span", { className: "rt-SegmentedControlItemSeparator" }), e21.createElement("span", { className: "rt-SegmentedControlItemLabel" }, e21.createElement("span", { className: "rt-SegmentedControlItemLabelActive" }, o42), e21.createElement("span", { className: "rt-SegmentedControlItemLabelInactive", "aria-hidden": true }, o42))));
...
var r17 = o35.forwardRef((t16, p26) => {
  const { asChild: e28, className: s20, ...m13 } = v(t16, t6, u, r), i18 = e28 ? dist_exports.Root : "section";
  return o35.createElement(i18, { ...m13, ref: p26, className: (0, import_classnames42.default)("rt-Section", s20) });
});
r17.displayName = "Section";
...
var f14 = e22.createContext({});
var C5 = (r24) => {
  const { children: o42, size: t16 = o3.size.default, ...s20 } = r24;
  return e22.createElement(dist_exports26.Root, { ...s20 }, e22.createElement(f14.Provider, { value: e22.useMemo(() => ({ size: t16 }), [t16]) }, o42));
};
C5.displayName = "Select.Root";
var u7 = e22.forwardRef((r24, o42) => {
  const t16 = e22.useContext(f14), { children: s20, className: p26, color: n20, radius: m13, placeholder: c4, ...S5 } = v({ size: t16?.size, ...r24 }, { size: o3.size }, i3, r);
  return e22.createElement(dist_exports26.Trigger, { asChild: true }, e22.createElement("button", { "data-accent-color": n20, "data-radius": m13, ...S5, ref: o42, className: (0, import_classnames43.default)("rt-reset", "rt-SelectTrigger", p26) }, e22.createElement("span", { className: "rt-SelectTriggerInner" }, e22.createElement(dist_exports26.Value, { placeholder: c4 }, s20)), e22.createElement(dist_exports26.Icon, { asChild: true }, e22.createElement(i8, { className: "rt-SelectIcon" }))));
});
u7.displayName = "Select.Trigger";
var g7 = e22.forwardRef((r24, o42) => {
  const t16 = e22.useContext(f14), { className: s20, children: p26, color: n20, container: m13, ...c4 } = v({ size: t16?.size, ...r24 }, { size: o3.size }, p10), S5 = H(), T6 = n20 || S5.accentColor;
  return e22.createElement(dist_exports26.Portal, { container: m13 }, e22.createElement(R2, { asChild: true }, e22.createElement(dist_exports26.Content, { "data-accent-color": T6, sideOffset: 4, ...c4, asChild: false, ref: o42, className: (0, import_classnames43.default)({ "rt-PopperContent": c4.position === "popper" }, "rt-SelectContent", s20) }, e22.createElement(dist_exports25.Root, { type: "auto", className: "rt-ScrollAreaRoot" }, e22.createElement(dist_exports26.Viewport, { asChild: true, className: "rt-SelectViewport" }, e22.createElement(dist_exports25.Viewport, { className: "rt-ScrollAreaViewport", style: { overflowY: void 0 } }, p26)), e22.createElement(dist_exports25.Scrollbar, { className: "rt-ScrollAreaScrollbar rt-r-size-1", orientation: "vertical" }, e22.createElement(dist_exports25.Thumb, { className: "rt-ScrollAreaThumb" }))))));
});
g7.displayName = "Select.Content";
var v9 = e22.forwardRef((r24, o42) => {
  const { className: t16, children: s20, ...p26 } = r24;
  return e22.createElement(dist_exports26.Item, { ...p26, asChild: false, ref: o42, className: (0, import_classnames43.default)("rt-SelectItem", t16) }, e22.createElement(dist_exports26.ItemIndicator, { className: "rt-SelectItemIndicator" }, e22.createElement(t9, { className: "rt-SelectItemIndicatorIcon" })), e22.createElement(dist_exports26.ItemText, null, s20));
});
v9.displayName = "Select.Item";
var y5 = e22.forwardRef(({ className: r24, ...o42 }, t16) => e22.createElement(dist_exports26.Group, { ...o42, asChild: false, ref: t16, className: (0, import_classnames43.default)("rt-SelectGroup", r24) }));
y5.displayName = "Select.Group";
var h3 = e22.forwardRef(({ className: r24, ...o42 }, t16) => e22.createElement(dist_exports26.Label, { ...o42, asChild: false, ref: t16, className: (0, import_classnames43.default)("rt-SelectLabel", r24) }));
h3.displayName = "Select.Label";
var I5 = e22.forwardRef(({ className: r24, ...o42 }, t16) => e22.createElement(dist_exports26.Separator, { ...o42, asChild: false, ref: t16, className: (0, import_classnames43.default)("rt-SelectSeparator", r24) }));
...
var o36 = r18.forwardRef((p26, e28) => {
  const { className: t16, color: a22, decorative: s20, ...m13 } = v(p26, t7, r);
  return r18.createElement("span", { "data-accent-color": a22, role: s20 ? void 0 : "separator", ...m13, ref: e28, className: (0, import_classnames44.default)("rt-Separator", t16) });
});
o36.displayName = "Separator";
...
var r19 = parseFloat(o37.version) >= 19 || "";

// node_modules/@radix-ui/themes/dist/esm/components/skeleton.js
var r20 = o38.forwardRef((t16, p26) => {
  const { children: e28, className: n20, loading: s20, ...m13 } = v(t16, p11, r);
  if (!s20) return e28;
  const i18 = o38.isValidElement(e28) ? dist_exports.Root : "span";
  return o38.createElement(i18, { ref: p26, "aria-hidden": true, className: (0, import_classnames45.default)("rt-Skeleton", n20), "data-inline-skeleton": o38.isValidElement(e28) ? void 0 : true, tabIndex: -1, inert: r19, ...m13 }, e28);
});
r20.displayName = "Skeleton";
...
var s18 = e23.forwardRef((i18, a22) => {
  const { className: d10, color: l10, radius: m13, tabIndex: t16, ...r24 } = v(i18, a6, r);
  return e23.createElement(dist_exports27.Root, { "data-accent-color": l10, "data-radius": m13, ref: a22, ...r24, asChild: false, className: (0, import_classnames46.default)("rt-SliderRoot", d10) }, e23.createElement(dist_exports27.Track, { className: "rt-SliderTrack" }, e23.createElement(dist_exports27.Range, { className: (0, import_classnames46.default)("rt-SliderRange", { "rt-high-contrast": i18.highContrast }), "data-inverted": r24.inverted ? "" : void 0 })), (r24.value ?? r24.defaultValue ?? []).map((S5, n20) => e23.createElement(dist_exports27.Thumb, { key: n20, className: "rt-SliderThumb", ...t16 !== void 0 ? { tabIndex: t16 } : void 0 })));
});
s18.displayName = "Slider";
...
var r21 = o39.forwardRef((t16, e28) => {
  const { asChild: p26, className: s20, ...n20 } = v(t16, p12), m13 = p26 ? dist_exports.Root : "strong";
  return o39.createElement(m13, { ...n20, ref: e28, className: (0, import_classnames47.default)("rt-Strong", s20) });
});
r21.displayName = "Strong";
...
var i16 = t13.forwardRef((o42, p26) => {
  const { className: s20, color: m13, radius: c4, ...a22 } = v(o42, a7, r);
  return t13.createElement(dist_exports28.Root, { "data-accent-color": m13, "data-radius": c4, ...a22, asChild: false, ref: p26, className: (0, import_classnames48.default)("rt-reset", "rt-SwitchRoot", s20) }, t13.createElement(dist_exports28.Thumb, { className: (0, import_classnames48.default)("rt-SwitchThumb", { "rt-high-contrast": o42.highContrast }) }));
});
i16.displayName = "Switch";
...
var N6 = o40.forwardRef((t16, r24) => {
  const { children: a22, className: s20, color: n20, ...p26 } = v(t16, p13, r);
  return o40.createElement(dist_exports20.Root, { className: "rt-TabNavRoot", "data-accent-color": n20, ...p26, asChild: false, ref: r24 }, o40.createElement(dist_exports20.List, { className: (0, import_classnames49.default)("rt-reset", "rt-BaseTabList", "rt-TabNavList", s20) }, a22));
});
N6.displayName = "TabNav.Root";
var v10 = o40.forwardRef((t16, r24) => {
  const { asChild: a22, children: s20, className: n20, ...p26 } = t16;
  return o40.createElement(dist_exports20.Item, { className: "rt-TabNavItem" }, o40.createElement(dist_exports20.Link, { ...p26, ref: r24, className: (0, import_classnames49.default)("rt-reset", "rt-BaseTabListTrigger", "rt-TabNavLink", n20), onSelect: void 0, asChild: a22 }, d3({ asChild: a22, children: s20 }, (i18) => o40.createElement(o40.Fragment, null, o40.createElement("span", { className: "rt-BaseTabListTriggerInner rt-TabNavLinkInner" }, i18), o40.createElement("span", { className: "rt-BaseTabListTriggerInnerHidden rt-TabNavLinkInnerHidden" }, i18)))));
});
...
var m11 = e24.forwardRef((o42, l10) => {
  const { layout: a22, ...r24 } = n6, { className: C7, children: c4, layout: y7, ...i18 } = v(o42, r24, r), w3 = g({ value: y7, className: n6.layout.className, propValues: n6.layout.values });
  return e24.createElement("div", { ref: l10, className: (0, import_classnames50.default)("rt-TableRoot", C7), ...i18 }, e24.createElement(c2, null, e24.createElement("table", { className: (0, import_classnames50.default)("rt-TableRootTable", w3) }, c4)));
});
m11.displayName = "Table.Root";
var d8 = e24.forwardRef(({ className: o42, ...l10 }, a22) => e24.createElement("thead", { ...l10, ref: a22, className: (0, import_classnames50.default)("rt-TableHeader", o42) }));
d8.displayName = "Table.Header";
var b4 = e24.forwardRef(({ className: o42, ...l10 }, a22) => e24.createElement("tbody", { ...l10, ref: a22, className: (0, import_classnames50.default)("rt-TableBody", o42) }));
b4.displayName = "Table.Body";
var P7 = e24.forwardRef((o42, l10) => {
  const { className: a22, ...r24 } = v(o42, u4);
  return e24.createElement("tr", { ...r24, ref: l10, className: (0, import_classnames50.default)("rt-TableRow", a22) });
});
P7.displayName = "Table.Row";
var T5 = e24.forwardRef((o42, l10) => {
  const { className: a22, ...r24 } = v(o42, f2);
  return e24.createElement("td", { className: (0, import_classnames50.default)("rt-TableCell", a22), ref: l10, ...r24 });
});
T5.displayName = "Table.Cell";
var f16 = e24.forwardRef((o42, l10) => {
  const { className: a22, ...r24 } = v(o42, f2);
  return e24.createElement("th", { className: (0, import_classnames50.default)("rt-TableCell", "rt-TableColumnHeaderCell", a22), scope: "col", ref: l10, ...r24 });
});
f16.displayName = "Table.ColumnHeaderCell";
var R5 = e24.forwardRef((o42, l10) => {
  const { className: a22, ...r24 } = v(o42, f2);
  return e24.createElement("th", { className: (0, import_classnames50.default)("rt-TableCell", "rt-TableRowHeaderCell", a22), scope: "row", ref: l10, ...r24 });
});
...
var m12 = s19.forwardRef((t16, o42) => {
  const { className: r24, ...e28 } = v(t16, r);
  return s19.createElement(dist_exports29.Root, { ...e28, ref: o42, className: (0, import_classnames51.default)("rt-TabsRoot", r24) });
});
m12.displayName = "Tabs.Root";
var b5 = s19.forwardRef((t16, o42) => {
  const { className: r24, color: e28, ...n20 } = v(t16, p13, r);
  return s19.createElement(dist_exports29.List, { "data-accent-color": e28, ...n20, asChild: false, ref: o42, className: (0, import_classnames51.default)("rt-BaseTabList", "rt-TabsList", r24) });
});
b5.displayName = "Tabs.List";
var P8 = s19.forwardRef((t16, o42) => {
  const { className: r24, children: e28, ...n20 } = t16;
  return s19.createElement(dist_exports29.Trigger, { ...n20, asChild: false, ref: o42, className: (0, import_classnames51.default)("rt-reset", "rt-BaseTabListTrigger", "rt-TabsTrigger", r24) }, s19.createElement("span", { className: "rt-BaseTabListTriggerInner rt-TabsTriggerInner" }, e28), s19.createElement("span", { className: "rt-BaseTabListTriggerInnerHidden rt-TabsTriggerInnerHidden" }, e28));
});
P8.displayName = "Tabs.Trigger";
var f17 = s19.forwardRef((t16, o42) => {
  const { className: r24, ...e28 } = v(t16, r);
  return s19.createElement(dist_exports29.Content, { ...e28, ref: o42, className: (0, import_classnames51.default)("rt-TabsContent", r24) });
});
...
var r23 = e25.forwardRef((t16, o42) => {
  const { className: p26, color: a22, radius: s20, style: m13, ...n20 } = v(t16, a8, r);
  return e25.createElement("div", { "data-accent-color": a22, "data-radius": s20, className: (0, import_classnames52.default)("rt-TextAreaRoot", p26), style: m13 }, e25.createElement("textarea", { className: "rt-reset rt-TextAreaInput", ref: o42, ...n20 }));
});
r23.displayName = "TextArea";
...
var u8 = t15.forwardRef((r24, s20) => {
  const e28 = t15.useRef(null), { children: l10, className: i18, color: p26, radius: f18, style: x4, ...P9 } = v(r24, f3, r);
  return t15.createElement("div", { "data-accent-color": p26, "data-radius": f18, style: x4, className: (0, import_classnames53.default)("rt-TextFieldRoot", i18), onPointerDown: (T6) => {
    const n20 = T6.target;
    if (n20.closest("input, button, a")) return;
    const o42 = e28.current;
    if (!o42) return;
    const a22 = n20.closest(`
            .rt-TextFieldSlot[data-side='right'],
            .rt-TextFieldSlot:not([data-side='right']) ~ .rt-TextFieldSlot:not([data-side='left'])
          `) ? o42.value.length : 0;
    requestAnimationFrame(() => {
      try {
        o42.setSelectionRange(a22, a22);
      } catch {
      }
      o42.focus();
    });
  } }, t15.createElement("input", { spellCheck: "false", ...P9, ref: composeRefs(e28, s20), className: "rt-reset rt-TextFieldInput" }), l10);
});
u8.displayName = "TextField.Root";
var c3 = t15.forwardRef((r24, s20) => {
  const { className: e28, color: l10, side: i18, ...p26 } = v(r24, i4);
  return t15.createElement("div", { "data-accent-color": l10, "data-side": i18, ...p26, ref: s20, className: (0, import_classnames53.default)("rt-TextFieldSlot", e28) });
});
...
var N7 = e26.forwardRef(({ defaultOpen: n20 = true, ...a22 }, l10) => {
  const [c4, h4] = e26.useState(n20);
  return e26.createElement(z, { ...a22, ref: l10, open: c4, onOpenChange: h4 });
});
N7.displayName = "ThemePanel";
var z = e26.forwardRef((n20, a22) => {
  const { open: l10, onOpenChange: c4, onAppearanceChange: h4, ...v11 } = n20, G6 = H(), { appearance: d10, onAppearanceChange: B5, accentColor: g8, onAccentColorChange: $, grayColor: f18, onGrayColorChange: F, panelBackground: T6, onPanelBackgroundChange: O2, radius: w3, onRadiusChange: D3, scaling: x4, onScalingChange: j } = G6, E5 = h4 !== void 0, K2 = useCallbackRef(h4), k3 = e26.useCallback((t16) => {
    const o42 = a92();
    if (d10 !== "inherit") {
      B5(t16);
      return;
    }
    E5 ? K2(t16) : (H3(t16), l92(t16)), o42();
  }, [d10, B5, E5, K2]), P9 = a9(g8), S5 = f18 === "auto" ? P9 : f18, [W2, M2] = e26.useState("idle");
  async function U() {
    const t16 = { appearance: d10 === s2.appearance.default ? void 0 : d10, accentColor: g8 === s2.accentColor.default ? void 0 : g8, grayColor: f18 === s2.grayColor.default ? void 0 : f18, panelBackground: T6 === s2.panelBackground.default ? void 0 : T6, radius: w3 === s2.radius.default ? void 0 : w3, scaling: x4 === s2.scaling.default ? void 0 : x4 }, o42 = Object.keys(t16).filter((i18) => t16[i18] !== void 0).map((i18) => `${i18}="${t16[i18]}"`).join(" "), m13 = o42 ? `<Theme ${o42}>` : "<Theme>";
    M2("copying"), await navigator.clipboard.writeText(m13), M2("copied"), setTimeout(() => M2("idle"), 2e3);
  }
  const [Z, H3] = e26.useState(d10 === "inherit" ? null : d10), b6 = `
      [contenteditable],
      [role="combobox"],
      [role="listbox"],
      [role="menu"],
      ...
      textarea
    `;
  return e26.useEffect(() => {
    function t16(o42) {
      const m13 = o42.altKey || o42.ctrlKey || o42.shiftKey || o42.metaKey, i18 = document.activeElement?.closest(b6);
      o42.key?.toUpperCase() === "T" && !m13 && !i18 && c4(!l10);
    }
    return document.addEventListener("keydown", t16), () => document.removeEventListener("keydown", t16);
  }, [c4, l10, b6]), e26.useEffect(() => {
    function t16(o42) {
      const m13 = o42.altKey || o42.ctrlKey || o42.shiftKey || o42.metaKey, i18 = document.activeElement?.closest(b6);
      o42.key?.toUpperCase() === "D" && !m13 && !i18 && k3(Z === "light" ? "dark" : "light");
    }
    return document.addEventListener("keydown", t16), () => document.removeEventListener("keydown", t16);
  }, [k3, Z, b6]), e26.useEffect(() => {
    const t16 = document.documentElement, o42 = document.body;
    function m13() {
      const y7 = t16.classList.contains("dark") || t16.classList.contains("dark-theme") || o42.classList.contains("dark") || o42.classList.contains("dark-theme");
      H3(d10 === "inherit" ? y7 ? "dark" : "light" : d10);
    }
    const i18 = new MutationObserver(function(y7) {
      y7.forEach(function(X2) {
        X2.attributeName === "class" && m13();
      });
    });
    return m13(), d10 === "inherit" && (i18.observe(t16, { attributes: true }), i18.observe(o42, { attributes: true })), () => i18.disconnect();
  }, [d10]), e26.createElement(R2, { asChild: true, radius: "medium", scaling: "100%" }, e26.createElement(p17, { direction: "column", position: "fixed", top: "0", right: "0", mr: "4", mt: "4", inert: l10 ? void 0 : r19, ...v11, ref: a22, style: { zIndex: 9999, overflow: "hidden", maxHeight: "calc(100vh - var(--space-4) - var(--space-4))", borderRadius: "var(--radius-4)", backgroundColor: "var(--color-panel-solid)", transformOrigin: "top center", transitionProperty: "transform, box-shadow", transitionDuration: "200ms", transitionTimingFunction: l10 ? "ease-out" : "ease-in", transform: l10 ? "none" : "translateX(105%)", boxShadow: l10 ? "var(--shadow-5)" : "var(--shadow-2)", ...n20.style } }, e26.createElement(c2, null, e26.createElement(p16, { flexGrow: "1", p: "5", position: "relative" }, e26.createElement(p16, { position: "absolute", top: "0", right: "0", m: "2" }, e26.createElement(e27, { content: "Press T to show/hide the Theme Panel", side: "bottom", sideOffset: 6 }, e26.createElement(r15, { asChild: true, size: "3", tabIndex: 0, className: "rt-ThemePanelShortcut" }, e26.createElement("button", { onClick: () => c4(!l10) }, "T")))), e26.createElement(r3, { size: "5", trim: "both", as: "h3", mb: "5" }, "Theme"), e26.createElement(p14, { id: "accent-color-title", as: "p", size: "2", weight: "medium", mt: "5" }, "Accent color"), e26.createElement(o16, { columns: "10", gap: "2", mt: "3", role: "group", "aria-labelledby": "accent-color-title" }, s2.accentColor.values.map((t16) => e26.createElement("label", { key: t16, className: "rt-ThemePanelSwatch", style: { backgroundColor: `var(--${t16}-9)` } }, e26.createElement(e27, { content: `${C6(t16)}${g8 === "gray" && S5 !== "gray" ? ` (${C6(S5)})` : ""}` }, e26.createElement("input", { className: "rt-ThemePanelSwatchInput", type: "radio", name: "accentColor", value: t16, checked: g8 === t16, onChange: (o42) => $(o42.target.value) }))))), e26.createElement(p17, { asChild: true, align: "center", justify: "between" }, e26.createElement(p14, { as: "p", id: "gray-color-title", size: "2", weight: "medium", mt: "5" }, "Gray color")), e26.createElement(o16, { columns: "10", gap: "2", mt: "3", role: "group", "aria-labelledby": "gray-color-title" }, s2.grayColor.values.map((t16) => e26.createElement(p17, { key: t16, asChild: true, align: "center", justify: "center" }, e26.createElement("label", { className: "rt-ThemePanelSwatch", style: { backgroundColor: t16 === "auto" ? `var(--${P9}-9)` : t16 === "gray" ? "var(--gray-9)" : `var(--${t16}-9)`, filter: t16 === "gray" ? "saturate(0)" : void 0 } }, e26.createElement(e27, { content: `${C6(t16)}${t16 === "auto" ? ` (${C6(P9)})` : ""}` }, e26.createElement("input", { className: "rt-ThemePanelSwatchInput", type: "radio", name: "grayColor", value: t16, checked: f18 === t16, onChange: (o42) => F(o42.target.value) })))))), e26.createElement(p14, { id: "appearance-title", as: "p", size: "2", weight: "medium", mt: "5" }, "Appearance"), e26.createElement(o16, { columns: "2", gap: "2", mt: "3", role: "group", "aria-labelledby": "appearance-title" }, ["light", "dark"].map((t16) => e26.createElement("label", { key: t16, className: "rt-ThemePanelRadioCard" }, e26.createElement("input", { className: "rt-ThemePanelRadioCardInput", type: "radio", name: "appearance", value: t16, checked: Z === t16, onChange: (o42) => k3(o42.target.value) }), e26.createElement(p17, { align: "center", justify: "center", height: "32px", gap: "2" }, t16 === "light" ? e26.createElement("svg", { width: "15", height: "15", viewBox: "0 0 15 15", fill: "none", xmlns: "http://www.w3.org/2000/svg", style: { margin: "0 -1px" } }, e26.createElement("path", { d: "M7.5 0C7.77614 0 8 0.223858 8 0.5V2.5C8 2.77614 7.77614 3 7.5 3C7.22386 3 7 2.77614 7 2.5V0.5C7 0.223858 7.22386 0 7.5 0ZM2.1967 2.1967C2.39196 2.00144 2.70854 2.00144 2.90381 2.1967L4.31802 3.61091C4.51328 3.80617 4.51328 4.12276 4.31802 4.31802C4.12276 4.51328 3.80617 4.51328 3.61091 4.31802L2.1967 2.90381C2.00144 2.70854 2.00144 2.39196 2.1967 2.1967ZM0.5 7C0.223858 7 0 7.22386 0 7.5C0 7.77614 0.223858 8 0.5 8H2.5C2.77614 8 3 7.77614 3 7.5C3 7.22386 2.77614 7 2.5 7H0.5ZM2.1967 12.8033C2.00144 12.608 2.00144 12.2915 2.1967 12.0962L3.61091 10.682C3.80617 10.4867 4.12276 10.4867 4.31802 10.682C4.51328 10.8772 4.51328 11.1938 4.31802 11.3891L2.90381 12.8033C2.70854 12.9986 2.39196 12.9986 2.1967 12.8033ZM12.5 7C12.2239 7 12 7.22386 12 7.5C12 7.77614 12.2239 8 12.5 8H14.5C14.7761 8 15 7.77614 15 7.5C15 7.22386 14.7761 7 14.5 7H12.5ZM10.682 4.31802C10.4867 4.12276 10.4867 3.80617 10.682 3.61091L12.0962 2.1967C12.2915 2.00144 12.608 2.00144 12.8033 2.1967C12.9986 2.39196 12.9986 2.70854 12.8033 2.90381L11.3891 4.31802C11.1938 4.51328 10.8772 4.51328 10.682 4.31802ZM8 12.5C8 12.2239 7.77614 12 7.5 12C7.22386 12 7 12.2239 7 12.5V14.5C7 14.7761 7.22386 15 7.5 15C7.77614 15 8 14.7761 8 14.5V12.5ZM10.682 10.682C10.8772 10.4867 11.1938 10.4867 11.3891 10.682L12.8033 12.0962C12.9986 12.2915 12.9986 12.608 12.8033 12.8033C12.608 12.9986 12.2915 12.9986 12.0962 12.8033L10.682 11.3891C10.4867 11.1938 10.4867 10.8772 10.682 10.682ZM5.5 7.5C5.5 6.39543 6.39543 5.5 7.5 5.5C8.60457 5.5 9.5 6.39543 9.5 7.5C9.5 8.60457 8.60457 9.5 7.5 9.5C6.39543 9.5 5.5 8.60457 5.5 7.5ZM7.5 4.5C5.84315 4.5 4.5 5.84315 4.5 7.5C4.5 9.15685 5.84315 10.5 7.5 10.5C9.15685 10.5 10.5 9.15685 10.5 7.5C10.5 5.84315 9.15685 4.5 7.5 4.5Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" })) : e26.createElement("svg", { width: "15", height: "15", viewBox: "0 0 15 15", fill: "none", xmlns: "http://www.w3.org/2000/svg", style: { margin: "0 -1px" } }, e26.createElement("path", { d: "M2.89998 0.499976C2.89998 0.279062 2.72089 0.0999756 2.49998 0.0999756C2.27906 0.0999756 2.09998 0.279062 2.09998 0.499976V1.09998H1.49998C1.27906 1.09998 1.09998 1.27906 1.09998 1.49998C1.09998 1.72089 1.27906 1.89998 1.49998 1.89998H2.09998V2.49998C2.09998 2.72089 2.27906 2.89998 2.49998 2.89998C2.72089 2.89998 2.89998 2.72089 2.89998 2.49998V1.89998H3.49998C3.72089 1.89998 3.89998 1.72089 3.89998 1.49998C3.89998 1.27906 3.72089 1.09998 3.49998 1.09998H2.89998V0.499976ZM5.89998 3.49998C5.89998 3.27906 5.72089 3.09998 5.49998 3.09998C5.27906 3.09998 5.09998 3.27906 5.09998 3.49998V4.09998H4.49998C4.27906 4.09998 4.09998 4.27906 4.09998 4.49998C4.09998 4.72089 4.27906 4.89998 4.49998 4.89998H5.09998V5.49998C5.09998 5.72089 5.27906 5.89998 5.49998 5.89998C5.72089 5.89998 5.89998 5.72089 5.89998 5.49998V4.89998H6.49998C6.72089 4.89998 6.89998 4.72089 6.89998 4.49998C6.89998 4.27906 6.72089 4.09998 6.49998 4.09998H5.89998V3.49998ZM1.89998 6.49998C1.89998 6.27906 1.72089 6.09998 1.49998 6.09998C1.27906 6.09998 1.09998 6.27906 1.09998 6.49998V7.09998H0.499976C0.279062 7.09998 0.0999756 7.27906 0.0999756 7.49998C0.0999756 7.72089 0.279062 7.89998 0.499976 7.89998H1.09998V8.49998C1.09998 8.72089 1.27906 8.89997 1.49998 8.89997C1.72089 8.89997 1.89998 8.72089 1.89998 8.49998V7.89998H2.49998C2.72089 7.89998 2.89998 7.72089 2.89998 7.49998C2.89998 7.27906 2.72089 7.09998 2.49998 7.09998H1.89998V6.49998ZM8.54406 0.98184L8.24618 0.941586C8.03275 0.917676 7.90692 1.1655 8.02936 1.34194C8.17013 1.54479 8.29981 1.75592 8.41754 1.97445C8.91878 2.90485 9.20322 3.96932 9.20322 5.10022C9.20322 8.37201 6.82247 11.0878 3.69887 11.6097C3.45736 11.65 3.20988 11.6772 2.96008 11.6906C2.74563 11.702 2.62729 11.9535 2.77721 12.1072C2.84551 12.1773 2.91535 12.2458 2.98667 12.3128L3.05883 12.3795L3.31883 12.6045L3.50684 12.7532L3.62796 12.8433L3.81491 12.9742L3.99079 13.089C4.11175 13.1651 4.23536 13.2375 4.36157 13.3059L4.62496 13.4412L4.88553 13.5607L5.18837 13.6828L5.43169 13.7686C5.56564 13.8128 5.70149 13.8529 5.83857 13.8885C5.94262 13.9155 6.04767 13.9401 6.15405 13.9622C6.27993 13.9883 6.40713 14.0109 6.53544 14.0298L6.85241 14.0685L7.11934 14.0892C7.24637 14.0965 7.37436 14.1002 7.50322 14.1002C11.1483 14.1002 14.1032 11.1453 14.1032 7.50023C14.1032 7.25044 14.0893 7.00389 14.0623 6.76131L14.0255 6.48407C13.991 6.26083 13.9453 6.04129 13.8891 5.82642C13.8213 5.56709 13.7382 5.31398 13.6409 5.06881L13.5279 4.80132L13.4507 4.63542L13.3766 4.48666C13.2178 4.17773 13.0353 3.88295 12.8312 3.60423L12.6782 3.40352L12.4793 3.16432L12.3157 2.98361L12.1961 2.85951L12.0355 2.70246L11.8134 2.50184L11.4925 2.24191L11.2483 2.06498L10.9562 1.87446L10.6346 1.68894L10.3073 1.52378L10.1938 1.47176L9.95488 1.3706L9.67791 1.2669L9.42566 1.1846L9.10075 1.09489L8.83599 1.03486L8.54406 0.98184ZM10.4032 5.30023C10.4032 4.27588 10.2002 3.29829 9.83244 2.40604C11.7623 3.28995 13.1032 5.23862 13.1032 7.50023C13.1032 10.593 10.596 13.1002 7.50322 13.1002C6.63646 13.1002 5.81597 12.9036 5.08355 12.5522C6.5419 12.0941 7.81081 11.2082 8.74322 10.0416C8.87963 10.2284 9.10028 10.3497 9.34928 10.3497C9.76349 10.3497 10.0993 10.0139 10.0993 9.59971C10.0993 9.24256 9.84965 8.94373 9.51535 8.86816C9.57741 8.75165 9.63653 8.63334 9.6926 8.51332C9.88358 8.63163 10.1088 8.69993 10.35 8.69993C11.0403 8.69993 11.6 8.14028 11.6 7.44993C11.6 6.75976 11.0406 6.20024 10.3505 6.19993C10.3853 5.90487 10.4032 5.60464 10.4032 5.30023Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" })), e26.createElement(p14, { size: "1", weight: "medium" }, C6(t16)))))), e26.createElement(p14, { id: "radius-title", as: "p", size: "2", weight: "medium", mt: "5" }, "Radius"), e26.createElement(o16, { columns: "5", gap: "2", mt: "3", role: "group", "aria-labelledby": "radius-title" }, s2.radius.values.map((t16) => e26.createElement(p17, { key: t16, direction: "column", align: "center" }, e26.createElement("label", { className: "rt-ThemePanelRadioCard" }, e26.createElement("input", { className: "rt-ThemePanelRadioCardInput", type: "radio", name: "radius", id: `theme-panel-radius-${t16}`, value: t16, checked: w3 === t16, onChange: (o42) => D3(o42.target.value) }), e26.createElement(R2, { asChild: true, radius: t16 }, e26.createElement(p16, { m: "3", width: "32px", height: "32px", style: { borderTopLeftRadius: t16 === "full" ? "80%" : "var(--radius-5)", backgroundImage: "linear-gradient(to bottom right, var(--accent-3), var(--accent-4))", borderTop: "2px solid var(--accent-a8)", borderLeft: "2px solid var(--accent-a8)" } }))), e26.createElement(p16, { asChild: true, pt: "2" }, e26.createElement(p14, { asChild: true, size: "1", color: "gray" }, e26.createElement("label", { htmlFor: `theme-panel-radius-${t16}` }, C6(t16))))))), e26.createElement(p14, { id: "scaling-title", as: "p", size: "2", weight: "medium", mt: "5" }, "Scaling"), e26.createElement(o16, { columns: "5", gap: "2", mt: "3", role: "group", "aria-labelledby": "scaling-title" }, s2.scaling.values.map((t16) => e26.createElement("label", { key: t16, className: "rt-ThemePanelRadioCard" }, e26.createElement("input", { className: "rt-ThemePanelRadioCardInput", type: "radio", name: "scaling", value: t16, checked: x4 === t16, onChange: (o42) => j(o42.target.value) }), e26.createElement(p17, { align: "center", justify: "center", height: "32px" }, e26.createElement(R2, { asChild: true, scaling: t16 }, e26.createElement(p17, { align: "center", justify: "center" }, e26.createElement(p14, { size: "1", weight: "medium" }, C6(t16)))))))), e26.createElement(p17, { mt: "5", align: "center", gap: "2" }, e26.createElement(p14, { id: "panel-background-title", as: "p", size: "2", weight: "medium" }, "Panel background"), e26.createElement(popover_exports.Root, null, e26.createElement(popover_exports.Trigger, null, e26.createElement(o24, { size: "1", variant: "ghost", color: "gray" }, e26.createElement(s5, { label: "Learn more about panel background options" }, e26.createElement("svg", { width: "15", height: "15", viewBox: "0 0 15 15", fill: "currentColor", xmlns: "http://www.w3.org/2000/svg" }, e26.createElement("path", { d: "M7.49991 0.876892C3.84222 0.876892 0.877075 3.84204 0.877075 7.49972C0.877075 11.1574 3.84222 14.1226 7.49991 14.1226C11.1576 14.1226 14.1227 11.1574 14.1227 7.49972C14.1227 3.84204 11.1576 0.876892 7.49991 0.876892ZM1.82707 7.49972C1.82707 4.36671 4.36689 1.82689 7.49991 1.82689C10.6329 1.82689 13.1727 4.36671 13.1727 7.49972C13.1727 10.6327 10.6329 13.1726 7.49991 13.1726C4.36689 13.1726 1.82707 10.6327 1.82707 7.49972ZM8.24992 4.49999C8.24992 4.9142 7.91413 5.24999 7.49992 5.24999C7.08571 5.24999 6.74992 4.9142 6.74992 4.49999C6.74992 4.08577 7.08571 3.74999 7.49992 3.74999C7.91413 3.74999 8.24992 4.08577 8.24992 4.49999ZM6.00003 5.99999H6.50003H7.50003C7.77618 5.99999 8.00003 6.22384 8.00003 6.49999V9.99999H8.50003H9.00003V11H8.50003H7.50003H6.50003H6.00003V9.99999H6.50003H7.00003V6.99999H6.50003H6.00003V5.99999Z", fillRule: "evenodd", clipRule: "evenodd" }))))), e26.createElement(popover_exports.Content, { size: "1", style: { maxWidth: 220 }, side: "top", align: "center" }, e26.createElement(p14, { as: "p", size: "2" }, "Whether Card and Table panels are translucent, showing some of the background behind them.")))), e26.createElement(o16, { columns: "2", gap: "2", mt: "3", role: "group", "aria-labelledby": "panel-background-title" }, s2.panelBackground.values.map((t16) => e26.createElement("label", { key: t16, className: "rt-ThemePanelRadioCard" }, e26.createElement("input", { className: "rt-ThemePanelRadioCardInput", type: "radio", name: "panelBackground", value: t16, checked: T6 === t16, onChange: (o42) => O2(o42.target.value) }), e26.createElement(p17, { align: "center", justify: "center", height: "32px", gap: "2" }, t16 === "solid" ? e26.createElement("svg", { width: "15", height: "15", viewBox: "0 0 15 15", fill: "none", xmlns: "http://www.w3.org/2000/svg", style: { margin: "0 -2px" } }, e26.createElement("path", { d: "M0.877075 7.49988C0.877075 3.84219 3.84222 0.877045 7.49991 0.877045C11.1576 0.877045 14.1227 3.84219 14.1227 7.49988C14.1227 11.1575 11.1576 14.1227 7.49991 14.1227C3.84222 14.1227 0.877075 11.1575 0.877075 7.49988ZM7.49991 1.82704C4.36689 1.82704 1.82708 4.36686 1.82708 7.49988C1.82708 10.6329 4.36689 13.1727 7.49991 13.1727C10.6329 13.1727 13.1727 10.6329 13.1727 7.49988C13.1727 4.36686 10.6329 1.82704 7.49991 1.82704Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" })) : e26.createElement("svg", { width: "15", height: "15", viewBox: "0 0 15 15", fill: "none", xmlns: "http://www.w3.org/2000/svg", style: { margin: "0 -2px" } }, e26.createElement("path", { opacity: ".05", d: "M6.78296 13.376C8.73904 9.95284 8.73904 5.04719 6.78296 1.62405L7.21708 1.37598C9.261 4.95283 9.261 10.0472 7.21708 13.624L6.78296 13.376Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" }), e26.createElement("path", { opacity: ".1", d: "M7.28204 13.4775C9.23929 9.99523 9.23929 5.00475 7.28204 1.52248L7.71791 1.2775C9.76067 4.9119 9.76067 10.0881 7.71791 13.7225L7.28204 13.4775Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" }), e26.createElement("path", { opacity: ".15", d: "M7.82098 13.5064C9.72502 9.99523 9.72636 5.01411 7.82492 1.50084L8.26465 1.26285C10.2465 4.92466 10.2451 10.085 8.26052 13.7448L7.82098 13.5064Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" }), e26.createElement("path", { opacity: ".2", d: "M8.41284 13.429C10.1952 9.92842 10.1957 5.07537 8.41435 1.57402L8.85999 1.34729C10.7139 4.99113 10.7133 10.0128 8.85841 13.6559L8.41284 13.429Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" }), e26.createElement("path", { opacity: ".25", d: "M9.02441 13.2956C10.6567 9.8379 10.6586 5.17715 9.03005 1.71656L9.48245 1.50366C11.1745 5.09919 11.1726 9.91629 9.47657 13.5091L9.02441 13.2956Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" }), e26.createElement("path", { opacity: ".3", d: "M9.66809 13.0655C11.1097 9.69572 11.1107 5.3121 9.67088 1.94095L10.1307 1.74457C11.6241 5.24121 11.6231 9.76683 10.1278 13.2622L9.66809 13.0655Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" }), e26.createElement("path", { opacity: ".35", d: "M10.331 12.7456C11.5551 9.52073 11.5564 5.49103 10.3347 2.26444L10.8024 2.0874C12.0672 5.42815 12.0659 9.58394 10.7985 12.9231L10.331 12.7456Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" }), e26.createElement("path", { opacity: ".4", d: "M11.0155 12.2986C11.9938 9.29744 11.9948 5.71296 11.0184 2.71067L11.4939 2.55603C12.503 5.6589 12.502 9.35178 11.4909 12.4535L11.0155 12.2986Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" }), e26.createElement("path", { opacity: ".45", d: "M11.7214 11.668C12.4254 9.01303 12.4262 5.99691 11.7237 3.34116L12.2071 3.21329C12.9318 5.95292 12.931 9.05728 12.2047 11.7961L11.7214 11.668Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" }), e26.createElement("path", { opacity: ".5", d: "M12.4432 10.752C12.8524 8.63762 12.8523 6.36089 12.4429 4.2466L12.9338 4.15155C13.3553 6.32861 13.3554 8.66985 12.9341 10.847L12.4432 10.752Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" }), e26.createElement("path", { d: "M0.877075 7.49988C0.877075 3.84219 3.84222 0.877045 7.49991 0.877045C11.1576 0.877045 14.1227 3.84219 14.1227 7.49988C14.1227 11.1575 11.1576 14.1227 7.49991 14.1227C3.84222 14.1227 0.877075 11.1575 0.877075 7.49988ZM7.49991 1.82704C4.36689 1.82704 1.82708 4.36686 1.82708 7.49988C1.82708 10.6329 4.36689 13.1727 7.49991 13.1727C10.6329 13.1727 13.1727 10.6329 13.1727 7.49988C13.1727 4.36686 10.6329 1.82704 7.49991 1.82704Z", fill: "currentColor", fillRule: "evenodd", clipRule: "evenodd" })), e26.createElement(p14, { size: "1", weight: "medium" }, C6(t16)))))), e26.createElement(o13, { mt: "5", style: { width: "100%" }, onClick: U }, W2 === "copied" ? "Copied" : "Copy Theme")))));
});
z.displayName = "ThemePanelImpl";
function a92() {
  const n20 = document.createElement("style");
  return n20.appendChild(document.createTextNode("*,*::before,*::after{-webkit-transition:none!important;-moz-transition:none!important;-o-transition:none!important;-ms-transition:none!important;transition:none!important}")), document.head.appendChild(n20), () => {
    window.getComputedStyle(document.body), setTimeout(() => {
      document.head.removeChild(n20);
    }, 1);
  };
}
function C6(n20) {
  return n20.charAt(0).toUpperCase() + n20.slice(1);
}
function l92(n20) {
  const a22 = document.documentElement, l10 = a22.classList.contains("light-theme"), c4 = a22.classList.contains("dark-theme"), h4 = a22.classList.contains("light"), v11 = a22.classList.contains("dark");
  (l10 || c4) && (a22.classList.remove("light-theme", "dark-theme"), a22.style.colorScheme = n20, a22.classList.add(`${n20}-theme`)), (h4 || v11) && (a22.classList.remove("light", "dark"), a22.style.colorScheme = n20, a22.classList.add(n20)), !l10 && !c4 && !h4 && !v11 && (a22.style.colorScheme = n20, a22.classList.add(n20));
}
...
var e27 = o41.forwardRef((i18, p26) => {
  const { children: r24, className: n20, open: s20, defaultOpen: l10, onOpenChange: m13, delayDuration: a22, disableHoverableContent: f18, content: P9, container: T6, forceMount: c4, ...d10 } = v(i18, e4), C7 = { open: s20, defaultOpen: l10, onOpenChange: m13, delayDuration: a22, disableHoverableContent: f18 };
  return o41.createElement(dist_exports33.Root, { ...C7 }, o41.createElement(dist_exports33.Trigger, { asChild: true }, r24), o41.createElement(dist_exports33.Portal, { container: T6, forceMount: c4 }, o41.createElement(R2, { asChild: true }, o41.createElement(dist_exports33.Content, { sideOffset: 4, collisionPadding: 10, ...d10, asChild: false, ref: p26, className: (0, import_classnames54.default)("rt-TooltipContent", n20) }, o41.createElement(p14, { as: "p", className: "rt-TooltipText", size: "1" }, P9), o41.createElement(dist_exports33.Arrow, { className: "rt-TooltipArrow" })))));
});
e27.displayName = "Tooltip";
export {
  s5 as AccessibleIcon,
  ...
  P as ThemeContext,
  N7 as ThemePanel,
  t9 as ThickCheckIcon,
  l5 as ThickChevronRightIcon,
  r9 as ThickDividerHorizontalIcon,
  e27 as Tooltip,
