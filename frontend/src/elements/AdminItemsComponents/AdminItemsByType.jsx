...
import "./adminItemsByType.css"

export default function AdminItemsByType({type}) {
    ...
    return (
        <>
            <Box className={"mainPageProductByIDSearchContainer"}>
                ...
                <FontAwesomeIcon icon={faAdd} className={"adminItemsByTypeAddButton"} size={"2x"}
                        ...
                        (isUsedFilter ? filteredProductsByID : productsByID).map((p) => (
                            <div key={p.id} className={"mainPageProductByIDContainer"}>
                                ...
                                <Box className={"mainPageProductByIDButtonAndBadgeContainer"}>
                                    <Button
                                        onClick={() => window.location.href = `/admin/redact/item/${p.id}`}>Редагувати</Button>
