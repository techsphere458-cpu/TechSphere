...
export default function AdminProducts() {
    ...
    useEffect(() => {
        if (searchTarget.trim().length > 0) {
            ...
            setFilteredProducts(products.filter(p => p.title.toLowerCase().trim().includes(searchTarget.toLowerCase().trim())));
        } else {
    ...
    return (
        <>
            <Box className={"adminProductSearchContainer"}>
                <FontAwesomeIcon icon={faAdd} className={"adminProductAddProductButton"} size={"2x"} onClick={() => navigate("/admin/add/product")}/>
                        ...
                        (isUsedFilter ? filteredProducts : products).map((p) => (
                            <div key={p.id} className={"adminProductContainer"}>
                                ...
                                <Button className={"adminProductButton"}
                                        onClick={() => navigate(`/admin/product/${encodeURIComponent(p.title.trim())}`)}>Більше</Button>
                                <button className={"adminProductsChangeButton"}
                                        onClick={() => navigate(`/admin/redact/product/${p.id}`)}>
