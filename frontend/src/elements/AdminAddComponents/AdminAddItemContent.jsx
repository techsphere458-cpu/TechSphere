...
import {Badge, Box, Flex, Separator, Switch, Select, Button, Tooltip} from "@radix-ui/themes";
            ...
            <Flex align={"center"} direction={"column"}>
                ...
                <Flex width={"100%"} height={"45%"} justify={"center"}>
                    <Flex justify={"center"} align={"center"} direction={"column"}
                        ...
                        <Badge className={"adminItemRedactContainerBadge"} variant="solid" radius="full"
                               color={isNew ? "indigo" : "gray"} size={"3"}
                ...
                <Flex align={"center"} direction={"column"} className={"adminAddItemContentPropsContainer"}>
                    ...
                    <Select.Root value={type} onValueChange={(value) => setType(value)}>
                        ...
                        <Select.Content>
                            {types.map(item => (
                                <Select.Item key={item.id} value={item?.title}>{item?.title}</Select.Item>
                            ))}
                        </Select.Content>
                    </Select.Root>
                    <Separator orientation={"horizontal"} size={"4"} style={{marginTop: "20px"}}/>

                    <Switch defaultChecked={urlInput} size={"3"} onCheckedChange={(checked) => setUrlInput(checked)}
                            style={{marginTop: "20px"}}/>
                    <Box style={{marginTop: "20px"}}>{urlInput ?
                        <CustomInputWithCleaner type={"text"} value={imageURlPrev} setValue={setImageURlPrev}
                                                tooltipText={"URL картинки"} placeholder={"URL картинки"}/>
                        :
                    ...
                    <Box style={{marginTop: "20px"}}>
                        <CustomInputWithCleaner type={"text"} value={model} placeholder={"Модель"}
                                                tooltipText={"Модель"} setValue={setModel}/>
                    </Box>
                    ...
                    <Box style={{marginTop: "20px"}}>
                        <CustomInputWithCleaner type={"number"} placeholder={"Ціна"} tooltipText={"Ціна"}
                                                setValue={setPrice} value={price}/>
                    </Box>

                    <Separator orientation={"horizontal"} size={"4"} style={{marginTop: "20px"}}/>

                    <Tooltip content={"Стан"}>
                        <Switch defaultChecked={isNew} onCheckedChange={(checked) => setIsNew(checked)}
                                style={{marginTop: "20px"}}/>
                    </Tooltip>
                    ...
                    <Flex style={{marginTop: "20px"}} gap={"3"}>
                        <AlertDialogCustom description={"Ви впевнені, що хочете додати цей продукт?"}
                                           buttonText={"Зберегти"} buttonColor={"green"}
                                           disabledText={correct ? "" : "Невірні настройки продукта"}
                                           onSaveClick={correct && saveItem} isLoading={isLoading}/>
                        <AlertDialogCustom description={"Ви впевнені, що хочете скинути настройки цього продукту?"}
                                           buttonText={"Скинути"} buttonColor={"red"} onSaveClick={clearAll} isLoading={isLoading}/>
