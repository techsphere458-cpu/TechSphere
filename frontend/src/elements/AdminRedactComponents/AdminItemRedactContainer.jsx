...
import { Badge, Box, Flex, Separator, Switch, Tooltip } from "@radix-ui/themes"
...
export default function AdminItemRedactContainer() {
    ...
    useEffect(() => {
        if (
            ...
            newImageURL &&
            newPrice &&
            newImageURL.length < 255 &&
            newModel.length > 0 &&
            newModel.length <= 30 &&
            newImageURL.length > 0 &&
            newPrice > 0 &&
            !newPrice.toString().includes("e", ",")
        ) {
            setCorrect(true)
        } else {
            setCorrect(false)
                ...
                <Flex width={"100%"} height={"45%"} justify={"center"}>
                    <Flex justify={"center"} align={"center"} direction={"column"} className={"adminItemRedactContainer"}>
                        ...
                        <Box style={{ marginTop: "350px", position: "absolute" }}>
                            <AlertDialogCustom
                                description={"Ви справді хочете видалити цей гаджет?"}
                ...
                <Flex width={"100%"} align={"center"} direction={"column"}>
                    <Flex className={"adminItemRedactContainerProps"} align={"center"} direction={"column"}>
                        <Flex width={"300px"} direction={"column"} align={"center"}>
                            <h3>Настройка</h3>
                            <Switch checked={urlInput} onCheckedChange={(checked) => setUrlInput(checked)} />
                            <Box className={"adminItemRedactContainerImageURLInput"}>
                                {urlInput ? (
                                    <CustomInputWithCleaner
                                        type={"text"}
                                        value={newImageURLPrev}
                            ...
                            <Tooltip content={"Стан"}>
                                <Switch
                                    checked={newIsNew}
                                    onCheckedChange={(checked) => setNewIsNew(checked)}
                            ...
                            <Flex style={{ marginTop: "20px" }} gap={"4"}>
                                <AlertDialogCustom
                                    description={"Ви точно хочете зберегти зміни?"}
                                    buttonColor={"green"}
                                    isLoading={pause}
                                    buttonText={"Зберегти"}
                                    onSaveClick={correct && saveAll}
                                    disabledText={!correct && "Ви ввели щось не правильно"}
                                />
                                <AlertDialogCustom
                                    description={"Ви точно хочете скинути всі зміни?"}
                                    buttonText={"Скинути"}
                                    buttonColor={"red"}
                                    onSaveClick={setDefaultAll}
                                    isLoading={pause}
                                />
                            </Flex>
                            <p></p>
